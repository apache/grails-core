...
abstract class AbstractGormApiRegistry<T extends AbstractDatastoreApi> {
     ...
     *
     * An API whose datastore cannot be read is deliberately kept: {@code getDatastore()} routes through a
     * {@link DatastoreResolver}, and a tenant-aware one throws whenever no tenant is bound on the calling
     * thread. That is an ordinary transient condition, not evidence the API belongs to this datastore, so
     * only a positive identity match evicts. Evicting on failure would permanently lose the entity's API
     * registration and leave subsequent lookups either failing or falling back to a different datastore.
     */
