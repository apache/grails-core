...
trait MultiTenant<D> extends Entity {
    ...
    static <T> T withTenant(Serializable tenantId, Closure<T> callable) {
        GormRegistry.instance.staticApiRegistry.findStaticApi((Class<D>) this).withTenant(tenantId, callable)
    }
    ...
    static <D> GormAllOperations eachTenant(Closure callable) {
        GormRegistry.instance.staticApiRegistry.findStaticApi((Class<D>) this, ConnectionSource.DEFAULT).eachTenant(callable)
    }
    ...
    static <D> GormAllOperations<D> withTenant(Serializable tenantId) {
        (GormAllOperations<D>) GormRegistry.instance.staticApiRegistry.findStaticApi((Class<D>) this).withTenant(tenantId)
    }
}
