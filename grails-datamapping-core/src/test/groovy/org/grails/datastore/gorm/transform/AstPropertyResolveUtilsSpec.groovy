...
class AstPropertyResolveUtilsSpec extends Specification {
    ...
    void "concurrent resolution of the exact same shared ClassNode instance from many threads is safe"() {
        ...
        // and idempotent, so even with the "synchronized (cachedClassProperties)" guard in
        ...
        // fix justified by IdentityHashMap's own documentation (it is not synchronized and concurrent
        // structural modification can corrupt it), not because this test can demonstrate its absence
        // breaking anything; this test instead guards against a regression to something observably
        // broken (an exception, a null, a wrong/partial result) under real concurrent load, which is
