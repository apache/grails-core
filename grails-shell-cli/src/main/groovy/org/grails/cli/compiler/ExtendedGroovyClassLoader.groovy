...
class ExtendedGroovyClassLoader extends GroovyClassLoader {
    ...
    @Override
    GroovyClassLoader.ClassCollector createCollector(CompilationUnit unit, SourceUnit su) {
        GroovyClassLoader.InnerLoader loader = getInnerLoader().run()
        return new ExtendedClassCollector(loader, unit, su)
    }

    private PrivilegedAction<GroovyClassLoader.InnerLoader> getInnerLoader() {
        return new PrivilegedAction<GroovyClassLoader.InnerLoader>() {
            @Override
    ...
    private static class DefaultScopeParentClassLoader extends ClassLoader {
        ...
        private static final String[] GROOVY_JARS_PREFIXES = [ 'groovy', 'antlr', 'asm' ] as String[]
        ...
        private void findGroovyJarsDirectly(ClassLoader classLoader, Set<URL> urls) {
            while (classLoader != null) {
                if (classLoader instanceof URLClassLoader) {
                    for (URL url in ((URLClassLoader) classLoader).getURLs()) {
        ...
        private boolean isGroovyJar(String entry) {
            ...
            for (String jarPrefix in GROOVY_JARS_PREFIXES) {
