 ...
 *
 */
// @CompileStatic here, not because this class is any less dynamic-by-design than before, but
// because a method invoked cold via reflection (as invokeMethod()/getProperty()/setProperty() are,
// being GroovyObject overrides reached from external dynamic Groovy code) that then makes a further
// dynamic self-call within the same class hits a Groovy indy/reflective call-site NPE
// ("MetaClass.initialize() because mc is null") -- see DefaultBeanConfiguration above for the same
// fix. External dynamic callers (the beans{} DSL, property assignment) are unaffected: Groovy always
// routes them through the overridden invokeMethod()/getProperty()/setProperty() below regardless of
// this class's own compile mode.
@CompileStatic
class BeanBuilder extends GroovyObjectSupport {

    private static final Log LOG = LogFactory.getLog(BeanBuilder)
    ...
    void importBeans(Resource resource) {
    ...
    Map<String, BeanDefinition> getBeanDefinitions() {
        ...
        for (String beanName in getSpringConfig().getBeanNames()) {
     ...
     * @since 0.4
     */
    // Not a synthesized GroovyObject: this class already hand-implements GroovyObject with its own
    // metaClass field, matching GrailsApplicationContext's earlier fix in this session.
    @POJO
    private class ConfigurableRuntimeBeanReference extends RuntimeBeanReference implements GroovyObject {
        ...
        private class WrappedPropertyValue extends GroovyObjectSupport {
            ...
            @SuppressWarnings('unused')
            boolean addAll(@SuppressWarnings('rawtypes') Collection values) {
            ...
            private void updateDeferredProperties(Object value) {
                if (value instanceof RuntimeBeanReference) {
                    BeanConfiguration outerBeanConfig = ConfigurableRuntimeBeanReference.this.beanConfig
                    BeanBuilder.this.deferredProperties.put(outerBeanConfig.getName(),
                            new DeferredProperty(outerBeanConfig, propertyName, propertyValue))
                }
            }
        }
     ...
     */
    void loadBeans(String resourcePattern) throws IOException {
    ...
    void loadBeans(Resource resource) {
        ...
        loadBeans([resource] as Resource[])
    }
    ...
    protected void finalizeDeferredProperties() {
        for (DeferredProperty dp in deferredProperties.values()) {
    ...
    protected BeanConfiguration invokeBeanDefiningMethod(String name, Object[] args) {
        ...
        if (args[0] instanceof Class) {
            ...
            if (args.length > 1 && args[1] instanceof Class) {
                ...
                for (Object o in namedArgs.keySet()) {
        ...
        if (hasClosureArgument) {
            ...
            InvokerHelper.invokeClosure(callable, [currentBeanConfig] as Object[])
        }
     ...
     */
    AbstractBeanDefinition bean(Class<?> type) {
