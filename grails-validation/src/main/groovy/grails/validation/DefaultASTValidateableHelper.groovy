...
@CompileStatic
class DefaultASTValidateableHelper implements ASTValidateableHelper {
    ...
    protected void addConstraintsField(final ClassNode classNode) {
        ...
        if (field == null || !field.getDeclaringClass().equals(classNode)) {
            classNode.addField(CONSTRAINED_PROPERTIES_PROPERTY_NAME,
                Modifier.STATIC | Modifier.PRIVATE, new ClassNode(Map),
    ...
    protected void addGetConstraintsMethod(final ClassNode classNode, boolean defaultNullable) {
        ...
        if (getConstraintsMethod == null || !getConstraintsMethod.getDeclaringClass().equals(classNode)) {
            ...
            final Expression getConstraintsMethodCall = new StaticMethodCallExpression(ClassHelper.make(ValidationSupport), 'getConstrainedPropertiesForClass', getConstrainedPropertiesForClassArguments)
            ...
            if (!defaultNullable) {
                ...
                for (final Map.Entry<String, ClassNode> entry : propertiesToConstrain.entrySet()) {
                    ...
                    final ConstructorCallExpression constrainedPropertyCtorCallExpression = new ConstructorCallExpression(
                            new ClassNode(ConstrainedProperty), constrainedPropertyConstructorArgumentList)
                    ...
                    final Expression constraintsMapContainsKeyExpression = new MethodCallExpression(
                            new VariableExpression(CONSTRAINED_PROPERTIES_PROPERTY_NAME, ClassHelper.make(Map)),
            ...
            final MethodNode methodNode = new MethodNode(getConstraintsMethodName, Modifier.STATIC | Modifier.PUBLIC, new ClassNode(Map), Parameter.EMPTY_ARRAY, ClassNode.EMPTY_ARRAY, methodBlockStatement)
    ...
    protected Map<String, ClassNode> getPropertiesToEnsureConstraintsFor(final ClassNode classNode) {
        ...
        for (final FieldNode field in allFields) {
            if (!field.isStatic()) {
        ...
        if (!superClass.equals(new ClassNode(Object))) {
    ...
    protected void addValidateMethod(final ClassNode classNode) {
        ...
        final MethodNode listArgValidateMethod = classNode.getMethod(VALIDATE_METHOD_NAME, [new Parameter(new ClassNode(List), fieldsToValidateParameterName)] as Parameter[])
        if (listArgValidateMethod == null) {
            ...
            final ClassNode validationSupportClassNode = ClassHelper.make(ValidationSupport)
            ...
            final Parameter fieldsToValidateParameter = new Parameter(new ClassNode(List), fieldsToValidateParameterName)
            MethodNode methodNode = new MethodNode(
                    VALIDATE_METHOD_NAME, Modifier.PUBLIC, ClassHelper.boolean_TYPE,
                    [fieldsToValidateParameter] as Parameter[], ClassNode.EMPTY_ARRAY, validateMethodCode)
            classNode.addMethod(methodNode)
            AnnotatedNodeUtils.markAsGenerated(classNode, methodNode)
        }
        ...
        if (noArgValidateMethod == null) {
            ...
            final ArgumentListExpression validateInstanceArguments = new ArgumentListExpression()
            validateInstanceArguments.addExpression(new CastExpression(new ClassNode(List), new ConstantExpression(null)))
