...
class DataBindingUtils {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(DataBindingUtils)
     ...
     */
    static void assignBidirectionalAssociations(Object object, Map source, PersistentEntity persistentEntity) {
        if (source == null) {
            return
        }

        for (Object key in source.keySet()) {
    ...
    private static List getPropertyNamesWithBindableValue(final Map constrainedProperties, final Boolean bindableValue) {
        ...
        for (Object entryObject in constrainedProperties.entrySet()) {
    ...
    private static boolean isSimpleType(final Class propertyType) {
        return propertyType != null && (propertyType.isPrimitive() || String.equals(propertyType) ||
                Boolean.equals(propertyType) || Character.equals(propertyType) || Number.isAssignableFrom(propertyType) ||
                BigInteger.equals(propertyType) || BigDecimal.equals(propertyType) || URL.equals(propertyType))
    }
    ...
    private static Map evaluateConstrainedProperties(final Class objectClass) {
        try {
            ...
            Object constrainedProperties = validationSupport.getMethod('getConstrainedPropertiesForClass', Class, boolean.class).invoke(null, objectClass, false)
            if (constrainedProperties instanceof Map) {
                return (Map) constrainedProperties
            }
        } catch (Exception ignored) {
    ...
    static Object getBindableConstraintValue(final Object constrainedProperty) {
        MetaClass metaClass = GroovySystem.getMetaClassRegistry().getMetaClass(constrainedProperty.getClass())
        try {
            Object value = metaClass.invokeMethod(constrainedProperty, 'getMetaConstraintValue', [DefaultASTDatabindingHelper.BINDABLE_CONSTRAINT_NAME] as Object[])
            if (value != null) {
                return value
            }
        } catch (Exception ignored) {
    ...
    static boolean isDenyByDefaultEnabled() {
        ...
        if (application != null) {
            return resolveDenyByDefault(
                    application.getConfig().getProperty(Settings.DATABINDING_DENY_BY_DEFAULT, Object, null))
        }
     ...
     * @return A BindingResult if there were errors or null if it was successful
     */
    static BindingResult bindObjectToDomainInstance(PersistentEntity entity, Object object, Object source) {
     ...
     */
    static BindingResult bindObjectToInstance(Object object, Object source, List include, List exclude, String filter, boolean clearMissing) {
    ...
    static BindingResult bindObjectToDomainInstance(PersistentEntity entity, Object object,
        ...
        if (entity != null && bindingResult != null) {
            ...
            for (Object error in bindingResult.getAllErrors()) {
    ...
    protected static String[] getMessageCodes(String messageCode,
            Class objectType) {
        String[] codes = [objectType.getName() + '.' + messageCode, messageCode] as String[]
        return codes
    }
    ...
    static DataBindingSource createDataBindingSource(GrailsApplication grailsApplication, Class bindingTargetType, Object bindingSource) {
    ...
    static CollectionDataBindingSource createCollectionDataBindingSource(GrailsApplication grailsApplication, Class bindingTargetType, Object bindingSource) {
        final DataBindingSourceRegistry registry = getDataBindingSourceRegistry(grailsApplication)
        final MimeType mimeType = getMimeType(grailsApplication, bindingSource)
        return registry.createCollectionDataBindingSource(mimeType, bindingTargetType, bindingSource)
    }

    static MimeType getMimeType(GrailsApplication grailsApplication,
    ...
    @SuppressWarnings('unchecked')
    static Map convertPotentialGStrings(Map<Object, Object> args) {
