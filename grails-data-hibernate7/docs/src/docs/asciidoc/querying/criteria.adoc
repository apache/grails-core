...
Eager fetching can be achieved using a criteria query:
...
def results = Airport.withCriteria {
    eq "region", "EMEA"
    fetchMode "flights", FetchMode.SELECT
}
----
...
*get*,Returns a unique result set i.e. just one row.
*scroll*,Returns a scrollable result set.
*listDistinct*,Allows listing only distinct entities.
