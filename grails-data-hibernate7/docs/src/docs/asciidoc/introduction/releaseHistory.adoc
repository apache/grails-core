...
* **HQL injection safety** — Single-argument `find`, `findAll`, `executeQuery`, and `executeUpdate` now enforce Groovy GString interpolation for safe parameterization; passing a plain `String` throws `UnsupportedOperationException`.
