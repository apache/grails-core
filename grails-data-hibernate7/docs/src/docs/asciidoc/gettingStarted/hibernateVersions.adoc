...
| HQL string safety
| Single-argument GORM HQL overloads are reserved for Groovy `GString` values so interpolated values can be bound safely.
| Pass a `GString` with interpolated values, or use the named-parameter or positional-parameter overload with an explicit params map or list. See <<querying-hql,HQL Queries>>.
