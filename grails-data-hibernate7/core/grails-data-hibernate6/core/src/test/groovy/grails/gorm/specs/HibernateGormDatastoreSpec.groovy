...
import org.apache.grails.data.hibernate6.core.GrailsDataHibernate6TckManager // Changed
...
import org.hibernate.internal.SessionFactoryImpl // Assuming compatible for H6
 ...
 */
class HibernateGormDatastoreSpec extends GrailsDataTckSpec<GrailsDataHibernate6TckManager> { // Changed
    ...
    HibernatePersistentEntity createPersistentEntity(GrailsDomainBinder binder
        ...
        class ${className} implements HibernateEntity<${className}> {
            ...
            static mapping = {
                ${staticMapping.collect { name, value -> "${name} ${name}" }.join('\n            ')}
            }
        }
    """
