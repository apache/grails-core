...
public class HibernateMappingContextSessionFactoryBean extends HibernateExceptionTranslator
  ...
  /**
   * Set a Hibernate NamingStrategy for the SessionFactory, determining the physical column and
   ...
   * objects as values. Instead of a single listener object, you can also pass in a list or set of
   * listeners objects as value.
   *
   * <p>See the Hibernate documentation for further details on listener types and associated
   * listener interfaces.
   *
   * @param eventListeners Map with listener type Strings as keys and listener objects as values
   */
  ...
  protected void buildSessionFactory() throws Exception {
    ...
    if (mappingResources != null) {
      ...
      for (String mapping : mappingResources) {
        Resource mr =
            new ClassPathResource(mapping.trim(), resourcePatternResolver.getClassLoader());
  ...
  protected HibernateMappingContextConfiguration newConfiguration() throws Exception {
    ...
    if (currentSessionContextClass != null) {
      config.setProperty(
          Environment.CURRENT_SESSION_CONTEXT_CLASS, currentSessionContextClass.getName());
    }
    return config;
  }
