...
public class HibernateLegacyEnumType<T extends Enum<T>>
  ...
  /**
   * An instance of this class is "configured" by a call to {@link #setParameterValues}, where
   * configuration parameters are given as entries in a {@link Properties} object. There are two
   * distinct ways an instance may be configured:
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>In the case of annotations or {@code orm.xml}, a {@link ParameterType} is passed to {@link
   * #setParameterValues} under the key {@value #PARAMETER_TYPE}.
   *
   * <p>But in the case of {@code hbm.xml}, there are multiple parameters:
   *
   * <ul>
   *   <li>{@value #ENUM}, the name of the Java enumeration class.
   *   <li>{@value #NAMED}, specifies if the enum should be mapped by name. Default is to map as
   *       ordinal.
   *   <li>{@value #TYPE}, a JDBC type code (legacy alternative to {@value #NAMED}).
   * </ul>
   */
  @Override
  public void setParameterValues(Properties parameters) {
    ...
    if (parameters.containsKey(TYPE)) {
      ...
      if (parameters.containsKey(NAMED)) {
        indicators =
            new LocalJdbcTypeIndicators(
                // use ORDINAL as default for hbm.xml mappings
                getBoolean(NAMED, parameters) ? STRING : ORDINAL, false, columnLength);
      } else {
        indicators =
            new LocalJdbcTypeIndicators(getEnumType(reader), isNationalized(reader), columnLength);
      }
  ...
  private jakarta.persistence.EnumType getEnumType(ParameterType reader) {
    if (reader != null) {
      if (reader.isPrimaryKey()) {
        final MapKeyEnumerated enumAnn =
  ...
  private void verifyConfigured() {
    if (enumJavaType == null) {
      throw new AssertionFailure(
          "EnumType (" + enumClass.getName() + ") not properly, fully configured");
    }
  }
  ...
  public String toSqlLiteral(T value) {
    verifyConfigured();
    return isOrdinal() ? Integer.toString(value.ordinal()) : "'" + value.name() + "'";
  }
  ...
  private class LocalJdbcTypeIndicators implements JdbcTypeIndicators {
    ...
    private LocalJdbcTypeIndicators(
        jakarta.persistence.EnumType enumType, boolean nationalized, Long columnLength) {
