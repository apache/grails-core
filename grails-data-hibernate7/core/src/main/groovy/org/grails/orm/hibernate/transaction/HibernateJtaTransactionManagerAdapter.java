...
/**
 * Adapter for adding transaction controlling hooks for supporting Hibernate's
 * org.hibernate.engine.transaction.Isolater class's interaction with transactions
 *
 * <p>This is required when there is no real JTA transaction manager in use and Spring's {@link
 * TransactionAwareDataSourceProxy} is used.
 *
 * <p>Without this solution, using Hibernate's TableGenerator identity strategies will fail to
 * support transactions. The id generator will commit the current transaction and break
 * transactional behaviour.
 *
 * <p>The javadoc of Hibernate's {@code TableHiLoGenerator} states this. However this isn't
 * mentioned in the javadocs of other TableGenerators.
 *
 * @author Lari Hotari
 */
public class HibernateJtaTransactionManagerAdapter implements TransactionManager {
  ...
  public HibernateJtaTransactionManagerAdapter(
      PlatformTransactionManager springTransactionManager) {
  ...
  public Transaction suspend() {
    currentTransactionHolder.set(
        springTransactionManager.getTransaction(
