...
public class PredicateGenerator {
  ...
  public Predicate[] getPredicates(
    ...
    List<Predicate> list =
        criteriaList.stream()
            .map(
                criterion ->
                    handleCriterion(cb, criteriaQuery, root_, fromsByProvider, entity, criterion))
  ...
  private Predicate handleJunction(
    ...
    var predicates =
        getPredicates(cb, criteriaQuery, root_, junction.getCriteria(), fromsByProvider, entity);
    ...
    if (Objects.nonNull(queryableCriteria)) {
      ...
      if (projection instanceof Query.PropertyProjection pp) {
        ...
        Predicate[] predicates2 =
            getPredicates(cb, criteriaQuery, from2, c.getValue().getCriteria(), newMap2, entity);
        subquery2
  ...
  private Predicate handleRLike(
      HibernateCriteriaBuilder cb, Path fullyQualifiedPath, Query.RLike c) {
  ...
  private CriteriaBuilder.In getQueryableCriteriaValue(
    ...
    var predicates =
        getPredicates(
            cb, criteriaQuery, from, queryableCriteria.getCriteria(), clonedProviderByName, entity);
    subquery
        .select(clonedProviderByName.getFullyQualifiedPath(subProperty))
        .distinct(true)
