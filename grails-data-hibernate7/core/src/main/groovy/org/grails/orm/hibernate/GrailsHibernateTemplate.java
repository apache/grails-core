...
public class GrailsHibernateTemplate implements IHibernateTemplate {
  ...
  @Override
  public <T1> T1 executeWithExistingOrCreateNewSession(
  ...
  /**
   * Never flush is a good strategy for read-only units of work. Hibernate will not track and look
  ...
  protected FlushMode applyFlushMode(Session session, boolean existingTransaction) {
    ...
    if (getFlushMode() == FLUSH_NEVER) {
      ...
      if (existingTransaction) {
        FlushMode previousFlushMode = session.getHibernateFlushMode();
        if (previousFlushMode.equals(FlushMode.AUTO)
            || previousFlushMode.equals(FlushMode.ALWAYS)) {
  ...
  protected DataAccessException convertJdbcAccessException(
      JDBCException ex, SQLExceptionTranslator translator) {
  ...
  public void setApplyFlushModeOnlyToNonExistingTransactions(
      boolean applyFlushModeOnlyToNonExistingTransactions) {
