 ...
 *
 * ⚠️ WARNING: This class contains REFLECTION-BASED HACKS and uses LEGACY HIBERNATE METHODS
 * that will be removed in Hibernate 8. This is a workaround for Hibernate 7 limitations.
 *
 * ISSUES WITH HIBERNATE 7:
 * - IncrementGenerator.configure() does not properly initialize all internal state
 * - physicalTableNames, sql, and previousValueHolder fields are not set correctly
 * - The initialize() method may not work as expected with the default configuration
 * - These issues appear to be fixed in later Hibernate versions but present in early Hibernate 7
 *
 * MIGRATION PATH FOR HIBERNATE 8:
 * 1. Check if IncrementGenerator.configure() properly initializes all fields
 * 2. Remove all reflection-based field access (returnClassField, columnField, etc.)
 * 3. Remove the manual initialize() method calls
 * 4. Remove the previousValueHolder initialization hack
 * 5. Simplify the generate() method to rely on parent class behavior
 * 6. Run full integration tests to verify increment ID generation works correctly
 *
...
public class GrailsIncrementGenerator extends IncrementGenerator {
    ...
    public GrailsIncrementGenerator(GeneratorCreationContext context, Identity mappedId, GrailsHibernatePersistentEntity domainClass) {
        ...
        if (tableName == null || tableName.isEmpty()) {
            tableName = domainClass.getJavaClass().getSimpleName().replaceAll("([a-z])([A-Z])", "$1_$2").toLowerCase();
        }
        ...
        if (columnName == null || columnName.contains(".")) {
            columnName = (mappedId != null && mappedId.getName() != null && !mappedId.getName().contains(".")) ? mappedId.getName() : "id";
        }
                ...
                if (!initialized) {
                    ...
                    try {
                        ...
                        if (sqlField.get(this) == null) {
                            sqlField.set(this, "select max(" + resolvedColumnName + ") from " + resolvedTableName);
                        }
                        ...
                        if (holderField.get(this) == null) {
                            java.lang.reflect.Method initMethod = clazz.getDeclaredMethod("initializePreviousValueHolder", SharedSessionContractImplementor.class);
