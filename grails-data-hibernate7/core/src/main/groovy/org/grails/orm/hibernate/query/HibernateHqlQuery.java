...
public class HibernateHqlQuery extends Query {
  ...
  public static HibernateHqlQuery createHqlQuery(
    ...
    Map namedParams = _namedParams != null ? new HashMap(_namedParams) : new HashMap();
    String queryString =
    ...
    Map args = MapUtils.isNotEmpty(_args) ? new HashMap(_args) : Collections.emptyMap();
  ...
  /**
   * Determine the number of top-level projections in the HQL query. Returns 0 if there is no
   * explicit SELECT clause (implicit entity projection), 1 if there is a single top-level
   * projection expression (including constructs like DISTINCT x or NEW map(...)), and 2 if there
   * are two or more top-level projection expressions (e.g. "select a, b from ...").
   *
   * <p>Notes: - Commas within parentheses or string literals are ignored. - Constructor expressions
   * like "new map(a as n, b as m)" count as a single projection. - Aggregate and function calls
  ...
  /**
   * If the HQL query has no alias in the FROM clause, inject a synthetic alias ("e") and qualify
   * the SELECT projection accordingly. Only SELECT is adjusted; the rest of the query is left
   * intact (WHERE/JOIN conditions are valid as-is in HQL).
   *
   * <p>Examples: "select nameType from NameType where lower(nameType)=:nameType" -> "select
   * e.nameType from NameType e where lower(nameType)=:nameType" "select NameType from NameType" ->
   * "select e from NameType e" "select name from Person" -> "select e.name from Person e"
   */
  private static String normalizeNonAliasedSelect(String hql) {
    ...
    if (cur + 2 < s.length()
        && s.substring(cur, Math.min(cur + 2, s.length())).equalsIgnoreCase("as")) {
      cur += 2;
      while (cur < s.length() && Character.isWhitespace(s.charAt(cur))) cur++;
    }
    ...
    if (projLower.startsWith("distinct ")) {
      prefix =
          selectClauseOrig.substring(
              0, selectClauseOrig.length() - projOrig.substring("distinct ".length()).length());
