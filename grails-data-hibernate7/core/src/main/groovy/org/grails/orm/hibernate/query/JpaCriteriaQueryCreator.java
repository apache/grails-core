...
public class JpaCriteriaQueryCreator {
  ...
  private List<Query.Projection> collectProjections() {
    return projections.getProjectionList().stream().filter(new ProjectionPredicate()).toList();
  }
  ...
  private JpaCriteriaQuery<?> createCriteriaQuery(List<Query.Projection> projections) {
    var cq =
        projections.stream()
                    .filter(
                        it ->
                            !(it instanceof Query.DistinctProjection
  ...
  private <T> void assignProjections(
    ...
    if (!projectionExpressions.isEmpty()) {
      ...
      tupleCriteriaQuery.select(
          criteriaBuilder.tuple(projectionExpressions.toArray(new Selection<?>[0])));
    } else {
