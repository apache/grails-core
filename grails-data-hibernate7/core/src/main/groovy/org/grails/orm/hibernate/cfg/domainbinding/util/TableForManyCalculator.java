...
public class TableForManyCalculator {
  ...
  protected TableForManyCalculator(
      PersistentEntityNamingStrategy namingStrategy, BackticksRemover backticksRemover) {
    this.namingStrategy = namingStrategy;
  ...
  public String calculateTableForMany(GrailsHibernatePersistentProperty property) {
    ...
    if (domainClass == null) {
      throw new MappingException(
          "Expected an entity to be associated with the association ("
              + property
              + ") and none was found. ");
    }
    ...
    if (property instanceof HibernateManyToManyProperty property1) {
      ...
      if (association.isOwningSide()) {
        return backticksRemover.apply(left)
            + UNDERSCORE
            + backticksRemover.apply(propertyColumnName);
      }
