...
class ClassPropertiesBinderSpec extends HibernateGormDatastoreSpec {
    ...
    void "test bindClassProperties"() {
        ...
        when:
        binder.bindClassProperties(domainClass, persistentClass, mappings, sessionFactoryBeanName)

        then:
        1 * grailsPropertyBinder.bindProperty(persistentClass, prop1, mappings, sessionFactoryBeanName) >> value1
        1 * propertyFromValueCreator.createProperty(value1, prop1) >> hibernateProp1

        1 * grailsPropertyBinder.bindProperty(persistentClass, prop2, mappings, sessionFactoryBeanName) >> value2
