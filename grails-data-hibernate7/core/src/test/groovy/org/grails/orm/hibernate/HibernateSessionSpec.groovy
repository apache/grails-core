...
class HibernateSessionSpec extends HibernateGormDatastoreSpec {
    ...
    void "retrieveAll resolves an id supplied as a String, which Hibernate coerces for the query"() {
        given:
        def session = getSession()
        def b1 = new HSBook(title: "Coerce1")
        def b2 = new HSBook(title: "Coerce2")
        def id1 = session.persist(b1)
        def id2 = session.persist(b2)
        session.flush()
        session.clear()
        ...
        def results = session.retrieveAll(HSBook, id1.toString(), id2.toString())
    ...
    void "retrieveAll preserves the requested order, duplicates and a null slot for a missing id"() {
        given:
        def session = getSession()
        def b1 = new HSBook(title: "Order1")
        def b2 = new HSBook(title: "Order2")
        def id1 = session.persist(b1)
        def id2 = session.persist(b2)
        session.flush()
        session.clear()
        ...
        when:
        def results = session.retrieveAll(HSBook, id2, id1, id2, missingId)
