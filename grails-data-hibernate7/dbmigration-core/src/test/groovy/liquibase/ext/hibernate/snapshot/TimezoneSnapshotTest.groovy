...
@CompileStatic
class TimezoneSnapshotTest {

    @Test
    void testTimezoneColumns() throws Exception {
        ...
        assertThat(
                ...
                hasItems(
                        ...
                        allOf(
                                ...
                                hasDatabaseAttribute('type', DataType, hasProperty('typeName', equalTo('timestamp with timezone')))
                        ),
                        // LocalDateTime column should result in 'timestamp' type
                        allOf(
                                hasProperty('name', equalTo('timestamp2')),
                                hasDatabaseAttribute('type', DataType, hasProperty('typeName', equalTo('timestamp')))
                        ),
                        ...
                        allOf(
                                ...
                                hasDatabaseAttribute('type', DataType, hasProperty('typeName', equalToIgnoringCase('timestamp with timezone')))
                        )
                )
        )
    }
    ...
    private static <T> FeatureMatcher<DatabaseObject, T> hasDatabaseAttribute(String attribute, Class<T> type, Matcher<T> matcher) {
        return new FeatureMatcher<DatabaseObject, T>(matcher, attribute, attribute) {
