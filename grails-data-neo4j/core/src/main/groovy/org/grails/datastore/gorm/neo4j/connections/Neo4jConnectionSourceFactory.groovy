...
@CompileStatic
@Slf4j
class Neo4jConnectionSourceFactory extends AbstractConnectionSourceFactory<Driver, Neo4jConnectionSourceSettings> {

    /**
     * @return Whether the embedded server capability is available or not
     */
    static boolean isEmbeddedAvailable() {
        return ClassUtils.isPresent('org.neo4j.harness.ServerControls', Neo4jConnectionSourceFactory.getClassLoader())
    }
    ...
    @Override
    ConnectionSource<Driver, Neo4jConnectionSourceSettings> create(String name, Neo4jConnectionSourceSettings settings) {
