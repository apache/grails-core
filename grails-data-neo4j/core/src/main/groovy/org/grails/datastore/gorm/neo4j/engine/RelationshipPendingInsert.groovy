 ...
 */
@CompileStatic
class RelationshipPendingInsert extends PendingInsertAdapter<Object, Serializable> {
    ...
    private static final Logger log = LoggerFactory.getLogger(RelationshipPendingInsert)
    ...
    RelationshipPendingInsert(EntityAccess parent, Association association, Collection<Serializable> pendingInserts, Neo4jSession session, boolean isUpdate) {
    ...
    void run() {
        ...
        final boolean isRelationshipAssociation = Relationship.isAssignableFrom(graphParent.getJavaClass())
        ...
        if (isUpdate &&
            ...
            if (association instanceof DynamicToOneAssociation || association instanceof OneToOne) {
                ...
                deleteParams = Collections.singletonMap(CypherBuilder.START, (Object) Collections.singletonList(parentId))
            } else {
                cypher.append(graphChild.formatId(TO))
                deleteParams = Collections.singletonMap(CypherBuilder.START, (Object) targetIdentifiers)
            }
