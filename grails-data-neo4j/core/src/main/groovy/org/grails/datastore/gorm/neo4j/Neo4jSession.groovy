 ...
 */
@CompileStatic
class Neo4jSession extends AbstractSession<Session> {

    private static final RemovalListener<RelationshipUpdateKey, Collection<Serializable>> EXCEPTION_THROWING_INSERT_LISTENER = exceptionThrowingInsertListener()
    private static final Logger log = LoggerFactory.getLogger(Neo4jSession)
    /**
    ...
    protected ConcurrentMap<RelationshipUpdateKey, Collection<Serializable>> pendingRelationshipInserts = newPendingRelationshipMap()
    protected ConcurrentMap<RelationshipUpdateKey, Collection<Serializable>> pendingRelationshipDeletes = newPendingRelationshipMap()

    private static RemovalListener<RelationshipUpdateKey, Collection<Serializable>> exceptionThrowingInsertListener() {
        return (RelationshipUpdateKey key, Collection<Serializable> value, RemovalCause cause) -> {
    ...
    private static ConcurrentMap<RelationshipUpdateKey, Collection<Serializable>> newPendingRelationshipMap() {
        Caffeine builder = Caffeine.newBuilder()
        builder = builder.executor(Runnable::run)
        builder = builder.removalListener(EXCEPTION_THROWING_INSERT_LISTENER)
        builder = builder.maximumSize(5000)
        return (ConcurrentMap<RelationshipUpdateKey, Collection<Serializable>>) builder.build().asMap()
    }

    Neo4jSession(Datastore datastore, MappingContext mappingContext, ApplicationEventPublisher publisher, boolean stateless, Driver boltDriver) {
     ...
     * @param id          The id
     */
    void addPendingRelationshipInsert(Serializable parentId, Association association, Serializable id) {
     ...
     * @param id          The id
     */
    void addPendingRelationshipDelete(Serializable parentId, Association association, Serializable id) {
     ...
     */
    Neo4jEntityPersister getEntityPersister(Object o) {
    ...
    void disconnect() {
        if (isConnected()) {
            ...
            try {
                if (this.@transaction != null && !isSynchronizedWithTransaction) {
    ...
    protected void flushPendingUpdates(Map<PersistentEntity, Collection<PendingUpdate>> updates) {
        ...
        for (PersistentEntity entity in entities) {
            ...
            final boolean isVersioned = entity.isVersioned() && entity.hasProperty(GormProperties.VERSION, Long)

            for (PendingUpdate pendingUpdate in pendingUpdates) {
                ...
                for (String dirtyPropertyName in dirtyPropertyNames) {
    ...
    private void processPendingRelationshipUpdates(GraphPersistentEntity entity, EntityAccess access, Serializable id, List<PendingOperation<Object, Serializable>> cascadingOperations, boolean isUpdate) {
        ...
        for (Association association in entity.getAssociations()) {
        ...
        if (entity.hasDynamicAssociations()) {
            if (!pendingRelationshipDeletes.isEmpty()) {
                for (RelationshipUpdateKey relationshipUpdateKey in pendingRelationshipDeletes.keySet()) {
            ...
            if (!pendingRelationshipInserts.isEmpty()) {
                ...
                Collections.sort(relationshipUpdates, new Comparator<RelationshipUpdateKey>() {
                    @Override
                    int compare(RelationshipUpdateKey o1, RelationshipUpdateKey o2) {
                        return o1.association.getName().compareTo(o2.association.getName())
                    }
                })
                for (RelationshipUpdateKey relationshipUpdate in relationshipUpdates) {
    ...
    protected void flushPendingInserts(Map<PersistentEntity, Collection<PendingInsert>> inserts) {
        ...
        for (RelationshipUpdateKey relationshipUpdateKey in pendingRelationshipInserts.keySet()) {
    ...
    private void processInsertsForEntity(org.neo4j.driver.Transaction neo4jTransaction, GraphPersistentEntity entity, Map<PersistentEntity, Collection<PendingInsert>> inserts, List<PendingOperation<Object, Serializable>> cascadingOperations) {
        ...
        if (!entityInserts.isEmpty()) {
            ...
            if (hasDynamicLabels) {
                ...
                for (PendingInsert entityInsert in entityInserts) {
                            ...
                            if (relEntity.type().equals(type)) {
                                ...
                                if (id != null) {
                                    ...
                                    if (childIds != null) {
                                        ...
                                        for (PersistentProperty pp in relEntity.getPersistentProperties()) {
                                    ...
                                    if (association.isOwningSide() || isTree) {
                                        ...
                                        if (childIds != null) {
                                            ...
                                            if (pendingInserts != null) {
                                                ...
                                                for (PendingInsert pendingInsert in pendingInserts) {
                                                    ...
                                                    if (childIds.contains(childId)) {
                                                        ...
                                                        childRows.add(Collections.singletonMap(CypherBuilder.PROPS, (Object) childProperties))
                ...
                if (!associationMerges.isEmpty()) {
                    for (String merge in associationMerges.keySet()) {
    ...
    private boolean isVetoedAfterPreOperations(PendingInsert entityInsert) {
        ...
        for (PendingOperation preOperation in preOperations) {
            preOperation.run()
        }
    ...
    String buildEntityCreateOperation(PersistentEntity entity, PendingInsert entityInsert, Map<String, Object> params, List<PendingOperation<Object, Serializable>> cascadingOperations) {
    ...
    void buildEntityCreateOperation(StringBuilder createCypher, String index, PersistentEntity entity, PendingInsert entityInsert, Map<String, Object> params, List<PendingOperation<Object, Serializable>> cascadingOperations) {
    ...
    private Map<String, Object> readNodePropertiesForInsert(PendingInsert entityInsert, GraphPersistentEntity graphEntity, List<PersistentProperty> persistentProperties, EntityAccess access) {
        ...
        for (PersistentProperty pp in persistentProperties) {
    ...
    private void nullOutEmbeddedProperties(String embeddedPropertyName, Embedded<?> embedded, Map<String, Object> simpleProps) {
        for (PersistentProperty nested in embedded.getAssociatedEntity().getPersistentProperties()) {
    ...
    private void flattenEmbeddedProperties(String embeddedPropertyName, Embedded<?> embedded, Object embeddedValue, Map<String, Object> simpleProps) {
        ...
        for (PersistentProperty nested in embeddedEntity.getPersistentProperties()) {
            if (nested instanceof Simple) {
    ...
    protected void flushPendingDeletes(Map<PersistentEntity, Collection<PendingDelete>> pendingDeletes) {
        ...
        for (PersistentEntity entity in persistentEntities) {
    ...
    Neo4jTransaction assertTransaction() {
    ...
    private void startDefaultTransaction() {
        ...
        this.@transaction = new Neo4jTransaction(boltDriver, transactionDefinition, true)
    }
    ...
    private void persistDirtyButUnsavedInstances() {
        for (Map<Serializable, Object> cache in firstLevelCache.values()) {
            for (Object obj in cache.values()) {
    ...
    private static class RelationshipUpdateKey {
        ...
        RelationshipUpdateKey(Serializable id, Association association) {
            this.id = id
