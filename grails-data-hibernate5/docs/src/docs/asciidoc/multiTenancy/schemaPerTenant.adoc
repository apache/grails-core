...
If the `AllTenantsResolver` resolver is not implemented then GORM will use the configured link:../../../api/org/grails/orm/hibernate/support/SchemaHandler.html[SchemaHandler] to resolve all the schema names from the database which it will use for the tenants.
...
To do this it uses an instance link:../../../api/org/grails/orm/hibernate/support/SchemaHandler.html[SchemaHandler] which by default uses the `CREATE SCHEMA [name]` syntax, but can be overridden and customized for other database dialects as necessary.
