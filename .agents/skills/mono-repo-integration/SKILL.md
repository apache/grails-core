...
4. **Integrate authors into the publish plugin.** Merge the imported repo's developer/contributor list into the `contributor(...)` block in `build-logic/plugins/src/main/groovy/org/apache/grails/buildsrc/PublishPlugin.groovy` (keep it alphabetized; don't duplicate existing entries).
...
**Restructure** directories to the monorepo convention (each deployable module gets a clean folder; nested project dirs are mapped explicitly via `projectDir` in `settings.gradle`, so directory names can differ from project names).
...
- **Each module `build.gradle`**: keep the `plugins { ... }` block and dependencies, but (a) strip versions in favor of the BOM, (b) replace the `apply { from ... }` block to point at the **root** `gradle/*.gradle` files (`test-config.gradle`, `publish-config.gradle`, `docs-config.gradle`, `java-config.gradle`, `reproducible-config.gradle`, etc.). Delete the module's reference to any deleted per-repo gradle file. For dependencies on **other monorepo modules**, use **`project(':grails-...')` syntax** (not Maven coordinates) — these are the published Gradle projects building alongside this one (see Principle 10).
...
- **`build-logic/plugins/.../PublishPlugin.groovy`**: merge in the imported developers as `contributor('<id>', '<name>', project)` entries (alphabetical, dedup).
...
- **`.github/workflows/gradle.yml`**: add a CI job (or extend an existing matrix) that runs the new `only<Name>Tests` slice; provision any required service container (e.g. a Redis service) the tests need.
...
### Phase 4 — Relocate functional/example apps
- Move `grails-<name>/examples/*` and any `*-test-app` to `grails-test-examples/<name>/...`.
