...
class MappingUtils {
    ...
    private static String getSuffixForGetterOrSetter(String propertyName) {
        ...
        if (propertyName.length() > 1 &&
            ...
            suffix = String.valueOf(Character.toUpperCase(propertyName.charAt(0))) + propertyName.substring(1)
        }
        return suffix
    }

    static String getTargetKey(PersistentProperty property) {
    ...
    static Collection createConcreteCollection(Class interfaceType) {
        ...
        else if (interfaceType == SortedSet) {
            elements = new TreeSet()
        }
        else if (interfaceType == Queue) {
     ...
     */
    static Field getDeclaredField(Class javaClass, String propertyName) {
        Class current = javaClass
        while (current != null) {
            Field[] declaredFields = current.getDeclaredFields()
            for (Field declaredField in declaredFields) {
                if (declaredField.getName() == propertyName) {
    ...
    static Class getGenericTypeForProperty(Class javaClass, String propertyName) {
        ...
        if (declaredField != null) {
            ...
            if (genericType instanceof ParameterizedType) {
                ...
                if (len > 0) {
                    int i = 0
                    if (Map.isAssignableFrom(type) && len == 2) {
                        i++
                    }
    ...
    static Class getGenericTypeForMapProperty(Class javaClass, String propertyName, boolean isKeyType) {
        Class genericClass = null
    ...
    static Class getGenericType(Class propertyType) {
