...
abstract class NativeEntryEntityPersister<T, K> extends ThirdPartyCacheEntityPersister<T> {

    public static final String EMBEDDED_PREFIX = 'embedded:'
    protected ClassMapping classMapping

    NativeEntryEntityPersister(MappingContext mappingContext, PersistentEntity entity,
    ...
    abstract String getEntityFamily()
        ...
        for (PersistentProperty prop in props) {
            ...
            if (prop instanceof OneToMany) {
                ...
                if (oneToMany.isOwningSide() && oneToMany.doesCascade(CascadeType.REMOVE)) {
                    if (Collection.isAssignableFrom(oneToMany.getType())) {
                ...
                if (manyToMany.isOwningSide() && manyToMany.doesCascade(CascadeType.REMOVE)) {
                    ...
                    if (Collection.isAssignableFrom(manyToMany.getType())) {
    ...
    protected void cascadeAfterDelete(PersistentEntity persistentEntity, EntityAccess entityAccess,
        ...
        for (PersistentProperty prop in props) {
            ...
            if (prop instanceof Basic) {
                ...
                if (!(prop instanceof Embedded) && !(((Object) prop) instanceof EmbeddedCollection) &&
    ...
    protected final void deleteEntities(PersistentEntity persistentEntity, Iterable objects) {
        if (objects != null) {
            ...
            if (!keys.isEmpty()) {
                ...
                for (Object object in deleteList) {
    ...
    @Override
    final Object lock(Serializable id) throws CannotAcquireLockException {
        return lock(id, DEFAULT_TIMEOUT)
    }

    @Override
    final Object lock(Serializable id, int timeout) throws CannotAcquireLockException {
    ...
    Object createObjectFromNativeEntry(PersistentEntity persistentEntity, Serializable nativeKey, T nativeEntry) {
        PersistentEntity resolvedEntity = discriminatePersistentEntity(persistentEntity, nativeEntry)

        cacheNativeEntry(resolvedEntity, nativeKey, nativeEntry)

        Object obj = newEntityInstance(resolvedEntity)
        refreshObjectStateFromNativeEntry(resolvedEntity, obj, nativeKey, nativeEntry, false)
        return obj
    }

    Object createObjectFromEmbeddedNativeEntry(PersistentEntity persistentEntity, T nativeEntry) {
        PersistentEntity resolvedEntity = discriminatePersistentEntity(persistentEntity, nativeEntry)
        Object obj = newEntityInstance(resolvedEntity)
        refreshObjectStateFromNativeEntry(resolvedEntity, obj, null, nativeEntry, true)
        return obj
    }
            ...
            if (prop instanceof Simple) {
                ...
                if (embeddedCollection.isBidirectional() && inverseSide != null) {
                    ...
                    if (loadedInstances instanceof Collection) {
                        ...
                        for (Object embeddedInstance in embeddedInstancesCollection) {
                ...
                if (isEmbedded) {
                    ...
                    if (List.isAssignableFrom(association.getType())) {
                    ...
                    else if (Set.isAssignableFrom(association.getType())) {
                        ea.setPropertyNoConversion(association.getName(),
                    ...
                    if (indexer != null) {
                        currentKey = convertIdIfNecessary(getPersistentEntity(), currentKey)
                        if (isLazy) {
                            if (List.isAssignableFrom(association.getType())) {
                                ea.setPropertyNoConversion(association.getName(),
                                        new PersistentList(currentKey, session, indexer))
                            }
                            else if (SortedSet.isAssignableFrom(association.getType())) {
                                ea.setPropertyNoConversion(association.getName(),
                                        new PersistentSortedSet(currentKey, session, indexer))
                            }
                            else if (Set.isAssignableFrom(association.getType())) {
                                ea.setPropertyNoConversion(association.getName(),
                                        new PersistentSet(currentKey, session, indexer))
                            }
                        }
                        else {
                            if (indexer != null) {
                                List keys = indexer.query(currentKey)
                ...
                if (associatedEntity != null) {
                    ...
                    Collection cached = ((SessionImplementor) session).getCachedCollection(
                            persistentEntity, currentKey, manyToMany.getName())
                    if (cached == null) {
                        Collection collection
                        if (isLazy) {
                            Collection keys = getManyToManyKeys(persistentEntity, obj, currentKey,
                                    nativeEntry, manyToMany)
                            if (List.isAssignableFrom(manyToMany.getType())) {
                                collection = new PersistentList(keys, childType, session)
                                ea.setPropertyNoConversion(manyToMany.getName(), collection)
                            }
                            else if (Set.isAssignableFrom(manyToMany.getType())) {
                        ...
                        ((SessionImplementor) session).cacheCollection(
                                persistentEntity, currentKey, collection, manyToMany.getName())
                    }
                    else {
    ...
    static String createEmbeddedCacheEntryKey(Object instance) {
    ...
    protected Object convertBasicEntryValue(PersistentEntity persistentEntity, Basic prop, Object entryValue) {
        ...
        if (entryValue instanceof Map) {
            ...
            if (genericType != null) {
                ...
                for (Object o in nativeMap.entrySet()) {
                    Map.Entry entry = (Map.Entry) o
                    String key = (String) entry.getKey()
                    Object value = conversionService.convert(entry.getValue(), genericType)
            ...
            if (genericType != null) {
                ConversionService conversionService = getMappingContext().getConversionService()
                for (Object o in collectionValue) {
                    collection.add(conversionService.convert(o, genericType))
                }
            }
            else {
                // just hope they don't need converting!
    ...
    private void handleCustom(PersistentProperty prop, EntityAccess ea, T nativeEntry) {
        CustomTypeMarshaller customTypeMarshaller = (CustomTypeMarshaller) ((Custom) prop).getCustomTypeMarshaller()
        if (!customTypeMarshaller.supports(getMappingContext())) {
            return
        }
    ...
    protected Serializable persistEntity(final PersistentEntity persistentEntity, Object obj, boolean isInsert) {
        ...
        final Object instance = proxyFactory.unwrap(obj)
        final NativeEntryModifyingEntityAccess entityAccess = (NativeEntryModifyingEntityAccess) createEntityAccess(persistentEntity, instance, tmp)

        K k = readObjectIdentifier(entityAccess, persistentEntity.getMapping())

        boolean isUpdate = k != null && !isInsert
        if (isUpdate && !getSession().isDirty(instance)) {
        ...
        if (si.isPendingAlready(instance)) {
            return (Serializable) k
        }
        else {
            si.registerPending(instance)
        }

        boolean assignedId = isAssignedId(persistentEntity)
        if (isNotUpdateForAssignedId(persistentEntity, instance, isUpdate, assignedId, si)) {
            isUpdate = false
        }
        String family = getEntityFamily()

        if (!isUpdate) {
            tmp = createNewEntry(family, instance)
        ...
        for (PersistentProperty prop in props) {
            ...
            if ((prop instanceof Simple)) {
                ...
                if (propValue instanceof Collection) {
                    Collection associatedObjects = (Collection) propValue
                    if (isInitializedCollection(associatedObjects)) {
                        setManyToMany(persistentEntity, instance, e, manyToMany, associatedObjects, toManyKeys)
                    }
                }
            }
            else if (prop instanceof ToOne) {
                        ...
                        if (association.isForeignKeyInChild()) {
                            ...
                            if (association.isBidirectional()) {
                                ...
                                if (inverseSide != null) {
                                    ...
                                    inverseAccess.setProperty(inverseSide.getName(), instance)
                                }
                            }
                            if (association.doesCascade(CascadeType.PERSIST)) {
        ...
        if (!isUpdate) {
            ...
            final K updateId = k
            PendingOperation<T, K> postOperation = new PendingOperationAdapter<T, K>(persistentEntity, k, e) {
                void run() {
                    ...
                    for (OneToMany inverseCollection in inverseCollectionUpdates.keySet()) {
            ...
            ((PendingOperation<T, K>) pendingOperation).addCascadeOperation(postOperation)
    ...
    private AbstractPersistentCollection getPersistentCollection(Collection associatedObjects, Class associationType) {
        if (associatedObjects instanceof Set) {
            if (associatedObjects instanceof SortedSet) {
                return new PersistentSortedSet(associationType, getSession(), (SortedSet) associatedObjects)
            }
            return new PersistentSet(associationType, getSession(), associatedObjects)
        }
    ...
    protected void handleEmbeddedToMany(EntityAccess entityAccess, T e, PersistentProperty prop, String key) {
        ...
        if (embeddedInstances instanceof Map) {
            ...
            for (Object k in instances.keySet()) {
            ...
            for (Object instance in instances) {
    ...
    T createNativeObjectForEmbedded(Association embedded, Object instance) {
    ...
    protected T handleEmbeddedInstance(Association association, Object embeddedInstance, boolean includeNulls) {
        ...
        if (associatedEntity != null) {
            ...
            for (PersistentProperty persistentProperty in embeddedProperties) {
                if (persistentProperty instanceof Simple) {
                    ...
                    CustomTypeMarshaller customTypeMarshaller = (CustomTypeMarshaller) ((Custom) persistentProperty).getCustomTypeMarshaller()
                    if (customTypeMarshaller.supports(getMappingContext())) {
                    ...
                    if (inverseSide instanceof Embedded ||
                        ...
                        if (persistentProperty instanceof OneToMany) {
                            ...
                            if (propValue instanceof Collection) {
                                Collection associatedObjects = (Collection) propValue
                                List<Serializable> keys = session.persist(associatedObjects)
                                setManyToMany(associatedEntity, embeddedInstance, embeddedEntry, manyToMany, associatedObjects, (Map<Association, List<Serializable>>) Collections.emptyMap())
                            }
                        }
                    }
                }
    ...
    private void handleIndexing(boolean update, T e, Map<PersistentProperty, Object> toIndex,
        ...
        if (update) {
            ...
            boolean unindex = oldValue == null ? propValue != null : !oldValue.equals(propValue)
    ...
    private void updateToManyIndices(T nativeEntry, Object identifier, Map<Association, List<Serializable>> toManyKeys, boolean preIndex) {
        ...
        for (Association association in toManyKeys.keySet()) {
            if (association.doesCascade(CascadeType.PERSIST)) {
    ...
    private void updatePropertyIndices(Object identifier, Map<PersistentProperty, Object> valuesToIndex, Map<PersistentProperty, Object> valuesToDeindex) {
        ...
        for (PersistentProperty persistentProperty in valuesToIndex.keySet()) {
        ...
        for (PersistentProperty persistentProperty in valuesToDeindex.keySet()) {
            final PropertyValueIndexer indexer = getPropertyIndexer(persistentProperty)
     ...
     * @return An indexer
     */
    abstract AssociationIndexer getAssociationIndexer(T nativeEntry, Association association)
    ...
    protected class NativeEntryModifyingEntityAccess extends BeanEntityAccess {
        ...
        protected T nativeEntry
        private Map<PersistentProperty, Object> toIndex

        NativeEntryModifyingEntityAccess(PersistentEntity persistentEntity, Object entity) {
                ...
                if (toIndex != null && property != null) {
                    ...
                    if (pm != null && isPropertyIndexed(pm.getMappedForm())) {
                        if (property instanceof ToOne) {
                            ...
                            if (!association.isForeignKeyInChild()) {
                                NativeEntryEntityPersister associationPersister = (NativeEntryEntityPersister) getSession().getPersister(value)
                                if (associationPersister != null) {
                                    if (value == null) {
        ...
        void setNativeEntry(T nativeEntry) {
            this.nativeEntry = nativeEntry
        }

        void setToIndex(Map<PersistentProperty, Object> toIndex) {
    ...
    protected boolean doManualDirtyCheck(Object instance, Object entry) {
        ...
        for (PersistentProperty prop in props) {
            ...
            if (prop instanceof Simple || prop instanceof Basic || prop instanceof ToOne) {
                ...
                CustomTypeMarshaller marshaller = (CustomTypeMarshaller) ((Custom) prop).getCustomTypeMarshaller()
    ...
    protected boolean areEqual(Object oldValue, Object currentValue, String propName) {
        ...
        Object oldCompared = oldValue
        Object currentCompared = currentValue
        if ('version' == propName) {
            ...
            if (oldValue instanceof Number && currentValue instanceof Number) {
                oldCompared = ((Number) oldValue).longValue()
                currentCompared = ((Number) currentValue).longValue()
            }
            else {
                oldCompared = oldValue.toString()
                currentCompared = currentValue.toString()
            }
        }

        Class oldValueClass = oldCompared.getClass()
        if (!oldValueClass.isArray()) {
            if (oldCompared instanceof Float) {
                return Float.floatToIntBits((Float) oldCompared) == Float.floatToIntBits((Float) currentCompared)
            }
            if (oldCompared instanceof Double) {
                return Double.doubleToLongBits((Double) oldCompared) == Double.doubleToLongBits((Double) currentCompared)
            }
            return oldCompared.equals(currentCompared)
        }

        // check arrays

        if (oldCompared.getClass() != currentCompared.getClass()) {
            // different dimension
            return false
        }

        if (oldCompared instanceof long[]) {
            return Arrays.equals((long[]) oldCompared, (long[]) currentCompared)
        }

        if (oldCompared instanceof int[]) {
            return Arrays.equals((int[]) oldCompared, (int[]) currentCompared)
        }

        if (oldCompared instanceof short[]) {
            return Arrays.equals((short[]) oldCompared, (short[]) currentCompared)
        }

        if (oldCompared instanceof char[]) {
            return Arrays.equals((char[]) oldCompared, (char[]) currentCompared)
        }

        if (oldCompared instanceof byte[]) {
            return Arrays.equals((byte[]) oldCompared, (byte[]) currentCompared)
        }

        if (oldCompared instanceof double[]) {
            return Arrays.equals((double[]) oldCompared, (double[]) currentCompared)
        }

        if (oldCompared instanceof float[]) {
            return Arrays.equals((float[]) oldCompared, (float[]) currentCompared)
        }

        if (oldCompared instanceof boolean[]) {
            return Arrays.equals((boolean[]) oldCompared, (boolean[]) currentCompared)
        }

        return Arrays.equals((Object[]) oldCompared, (Object[]) currentCompared)
    }
