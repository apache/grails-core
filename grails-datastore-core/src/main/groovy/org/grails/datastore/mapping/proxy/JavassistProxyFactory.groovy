...
@CompileStatic
class JavassistProxyFactory implements org.grails.datastore.mapping.proxy.ProxyFactory {
        ...
        if (proxyClass == null) {
            ...
            pf.setFilter(new MethodFilter() {
                boolean isHandled(Method method) {
                    Traits.TraitBridge traitBridge = method.getAnnotation(Traits.TraitBridge)
                    if (traitBridge != null) {
    ...
    protected Class[] getProxyInterfaces() {
        return [EntityProxy, GroovyObject] as Class[]
    }
