...
@CompileStatic
class JpaMappingConfigurationStrategy extends GormMappingConfigurationStrategy {

    JpaMappingConfigurationStrategy(MappingFactory propertyFactory) {
        super(propertyFactory)
    }

    private boolean isJpaEntity(Class clazz) {
        return clazz.isAnnotationPresent(Entity)
    }
    ...
    List<PersistentProperty> getPersistentProperties(PersistentEntity entity, MappingContext context, ClassMapping classMapping, boolean includeIdentifiers) {
        ...
        for (MetaProperty metaProperty in cpf.getMetaProperties()) {
            PropertyDescriptor descriptor = propertyFactory.createPropertyDescriptor(entityClass, metaProperty)

            if (descriptor == null || descriptor.getPropertyType() == null || descriptor.getPropertyType() == Object) {
                // indexed property
                continue
            }
            ...
            if (hasAnnotation(readMethod, field, Transient)) {
                continue
            }
            ...
            if (hasAnnotation(readMethod, field, Id)) {
                persistentProperties.add(propertyFactory.createIdentity(entity, context, descriptor))
            }
            else if (hasAnnotation(readMethod, field, EmbeddedId)) {
                persistentProperties.add(propertyFactory.createIdentity(entity, context, descriptor))
            }
            else if (hasAnnotation(readMethod, field, Embedded)) {
            ...
            // otherwise if the type is a domain class establish relationship
                ...
                if (hasAnnotation(readMethod, field, GeneratedValue)) {
    ...
    protected Association establishRelationshipForCollection(PropertyDescriptor property, Field field, PersistentEntity entity, MappingContext context, boolean embedded) {
        ...
        if (hasAnnotation(readMethod, field, ManyToMany)) {
            association = propertyFactory.createManyToMany(entity, context, property)
            ManyToMany manyToMany = getAnnotation(readMethod, field, ManyToMany)
            if (!manyToMany.mappedBy().equals('')) {
                ...
                List<PropertyDescriptor> descriptors = referencedCpf.getPropertiesAssignableToType(Collection)
                if (descriptors.size() > 0) {
                    for (PropertyDescriptor descriptor in descriptors) {
                        ...
                        ManyToMany relatedManyToMany = getAnnotation(referencedCpf, descriptor, ManyToMany)
        ...
        else if (hasAnnotation(readMethod, field, OneToMany)) {
            association = propertyFactory.createOneToMany(entity, context, property)
            OneToMany oneToMany = getAnnotation(readMethod, field, OneToMany)
    ...
    private ToOne establishDomainClassRelationship(PersistentEntity entity, PropertyDescriptor property, Field field, MappingContext context, boolean embedded) {
        ...
        if (getAnnotation(readMethod, field, ManyToOne) != null) {
            association = propertyFactory.createManyToOne(entity, context, property)

            List<PropertyDescriptor> descriptors = relatedCpf.getPropertiesAssignableToType(Collection)
            ...
            if (descriptors.size() > 0) {
                for (PropertyDescriptor descriptor in descriptors) {
                    ...
                    OneToMany relatedOneToMany = getAnnotation(relatedCpf, descriptor, OneToMany)
            ...
            OneToOne oneToOne = getAnnotation(readMethod, field, OneToOne)
            if (oneToOne != null) {
                ...
                if (descriptors.size() > 0) {
                    for (PropertyDescriptor descriptor in descriptors) {
                        OneToOne relatedOneToOne = getAnnotation(relatedCpf, descriptor, OneToOne)
    ...
    @Override
    IdentityMapping getIdentityMapping(final ClassMapping classMapping) {
            ...
            String[] getIdentifierName() {
                ...
                for (MetaProperty metaProperty in cpf.getMetaProperties()) {
                    ...
                    if (pd != null) {
                        ...
                        if (hasAnnotation(cpf, pd, Id)) {
                            idProperties.add(metaProperty.getName())
                        }
                        else if (hasAnnotation(cpf, pd, EmbeddedId)) {
                            idProperties.add(metaProperty.getName())
                        }
                    }
                }
    ...
    @PackageScope
    def <A extends Annotation> boolean hasAnnotation(Method method, Field field, Class<A> type) {
        return (method != null && method.isAnnotationPresent(type)) || (field != null && field.isAnnotationPresent(type))
    }

    @PackageScope
    def <A extends Annotation> A getAnnotation(Method method, Field field, Class<A> type) {
    ...
    @PackageScope
    def <A extends Annotation> boolean hasAnnotation(ClassPropertyFetcher cpf, PropertyDescriptor descriptor, Class<A> type) {
        Method readMethod = descriptor.getReadMethod()
    ...
    @PackageScope
    def <A extends Annotation> A getAnnotation(ClassPropertyFetcher cpf, PropertyDescriptor descriptor, Class<A> type) {
        Method readMethod = descriptor.getReadMethod()
        if (readMethod != null && readMethod.isAnnotationPresent(type)) {
