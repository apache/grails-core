...
@CompileStatic
abstract class AbstractGormMappingFactory<R extends Entity, T extends Property> extends MappingFactory<R, T> {
     ...
     */
    void setVersionByDefault(boolean versionByDefault) {
    ...
    R createMappedForm(PersistentEntity entity) {
        if (entityToMapping.containsKey(entity)) {
            ...
            List<Object> values = ClassPropertyFetcher.getStaticPropertyValuesFromInheritanceHierarchy(entity.getJavaClass(), GormProperties.MAPPING, Object)
            ...
            List<Closure> constraintValues = ClassPropertyFetcher.getStaticPropertyValuesFromInheritanceHierarchy(entity.getJavaClass(), GormProperties.CONSTRAINTS, Closure)
            for (Closure value in constraintValues) {
    ...
    @Override
    boolean isTenantId(PersistentEntity entity, MappingContext context, PropertyDescriptor descriptor) {
