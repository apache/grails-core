...
class ClassPropertyFetcher {
    ...
    static ClassPropertyFetcher forClass(final Class c) {
    ...
    ClassPropertyFetcher(final Class clazz) {
        ...
        for (MetaProperty property in properties) {
    ...
    Object getPropertyValue(String name) {
        return getStaticPropertyValue(this.theMetaClass, name)
    }
    ...
    Object getPropertyValue(final Object instance, String name) {
        return getInstancePropertyValue(instance, name, this.theMetaClass)
    }

    static Object getInstancePropertyValue(Object instance, String name) {
        return getInstancePropertyValue(instance, name, GroovySystem.getMetaClassRegistry().getMetaClass(instance.getClass()))
    }

    def <T> T getStaticPropertyValue(String name, Class<T> c) {
        return returnOnlyIfInstanceOf(getPropertyValue(name), c)
    }

    static <T> T getStaticPropertyValue(Class clazz, String name, Class<T> requiredType) {
     ...
     */
    def <T> List<T> getStaticPropertyValuesFromInheritanceHierarchy(String name, Class<T> c) {
        return getStaticPropertyValuesFromInheritanceHierarchy(this.classInfo, name, c)
    }
     ...
     * @return The list, with 0+ values (never null). Do not modify the returned list.
     */
    static <T> List<T> getStaticPropertyValuesFromInheritanceHierarchy(Class theClass, String name, Class<T> requiredTyped) {
    ...
    def <T> T getPropertyValue(String name, Class<T> c) {
     ...
     */
    static PropertyDescriptor createPropertyDescriptor(Class declaringClass, MetaProperty property) {
        int modifiers = property.getModifiers()
    ...
    List<PropertyDescriptor> getPropertiesOfType(Class javaClass) {
        ...
        for (MetaProperty property in properties) {
            int modifiers = property.getModifiers()
            if (Modifier.isStatic(modifiers) || property.getName().contains('$') || property.getType() != javaClass) {
                continue
            }
    ...
    List<PropertyDescriptor> getPropertiesAssignableToType(Class assignableType) {
        List<MetaProperty> properties = theMetaClass.getProperties()
        List<PropertyDescriptor> propertyDescriptors = new ArrayList<>(2)
        for (MetaProperty property in properties) {
            int modifiers = property.getModifiers()
    ...
    @SuppressWarnings('unchecked')
    List<PropertyDescriptor> getPropertiesAssignableFromType(Class assignableType) {
        List<MetaProperty> properties = theMetaClass.getProperties()
        List<PropertyDescriptor> propertyDescriptors = new ArrayList<>(2)
        for (MetaProperty property in properties) {
            int modifiers = property.getModifiers()
