...
@CompileStatic
class GormMappingConfigurationStrategy implements MappingConfigurationStrategy {
    ...
    private List getCollectionStaticProperty(ClassPropertyFetcher cpf, String property) {
        List<Collection> colls = cpf.getStaticPropertyValuesFromInheritanceHierarchy(property, Collection)
        if (colls == null) {
            return Collections.emptyList()
        }
        List values = new ArrayList()
        for (Collection coll in colls) {
            values.addAll(coll)
        }
        return values
    }
    ...
    private Map getMapStaticProperty(ClassPropertyFetcher cpf, String property) {
        List<Map> maps = cpf.getStaticPropertyValuesFromInheritanceHierarchy(property, Map)
        if (maps == null) {
    ...
    private Set establishRelationshipOwners(ClassPropertyFetcher cpf) {
        ...
        owners.addAll(cpf.getStaticPropertyValuesFromInheritanceHierarchy(BELONGS_TO, Class))
    ...
    protected Association establishRelationshipForCollection(PropertyDescriptor property, PersistentEntity entity, MappingContext context, Map<String, Class> hasManyMap, Map mappedByMap, boolean embedded) {
        ...
        if (StringUtils.hasText(mappingProperty)) {
            ...
            if (pd == null) {
                pd = findProperty(referencedCpf.getPropertiesAssignableToType(Collection), mappingProperty)
            }
            ...
            if (!forceUnidirectional(property, mappedByMap)) {
                ...
                if (isRelationshipToMany(entity, relatedClassType, relatedClassRelationships)) {
                    ...
                    for (Object o in relatedClassRelationships.keySet()) {
                    ...
                    /*
                    Map classRelationships = cpf.getPropertyValue(HAS_MANY, Map)
                ...
                if (relatedClassPropertyType == null || Collection.isAssignableFrom(relatedClassPropertyType)) {
                    List<PropertyDescriptor> descriptors = getPropertiesAssignableFromType(entity.getJavaClass(), referencedCpf)
                    Map relatedMappedBy = referencedCpf.getStaticPropertyValue(GormProperties.MAPPED_BY, Map)
                    List referencedTransients = referencedCpf.getStaticPropertyValue(GormProperties.TRANSIENT, List)
                    List referencedEmbedded = referencedCpf.getStaticPropertyValue(GormProperties.EMBEDDED, List)
        ...
        else if (Collection.isAssignableFrom(relatedClassPropertyType) ||
                Map.isAssignableFrom(relatedClassPropertyType)) {
            // many-to-many
        ...
        if (many) {
            Map classRelationships = referencedCpf.getPropertyValue(HAS_MANY, Map)
    ...
    private List<PropertyDescriptor> getPropertiesAssignableFromType(Class type, ClassPropertyFetcher propertyFetcher) {
        ...
        for (PropertyDescriptor prop in props) {
            if (prop.getPropertyType() != null && !prop.getPropertyType().equals(Object)) {
                valid.add(prop)
            }
        }
        return valid
    ...
    private String findManyRelatedClassPropertyName(String propertyName,
        ...
        // retrieve the relationship property
        for (Object o in classRelationships.keySet()) {
            String currentKey = (String) o
    ...
    private ToOne establishDomainClassRelationship(PersistentEntity entity, PropertyDescriptor property, MappingContext context, Map hasOneMap, boolean embedded) {
                ...
                if (mappedByValue != null && property.getName().equals(mappedByValue)) {
                    ...
                    // on the mapping property
                        ...
                        if (mappedBy.containsValue(property.getName())) {
                            for (Object o in mappedBy.keySet()) {
        ...
        else if (!embedded && Collection.isAssignableFrom(relatedClassPropertyType) || Map.isAssignableFrom(relatedClassPropertyType)) {
    ...
    protected boolean isCollectionType(Class type) {
        return Collection.isAssignableFrom(type) || Map.isAssignableFrom(type)
    }
    ...
    private boolean isIdentifierProperty(String propertyName, String[] identifierName) {
        for (String n in identifierName) {
     ...
     */
    Map getAssociationMap(ClassPropertyFetcher cpf) {
    ...
    static boolean isAbstract(PersistentEntity entity) {
    ...
    IdentityMapping getDefaultIdentityMapping(final ClassMapping classMapping) {
