...
abstract class AbstractMappingContext implements MappingContext, Initializable {
    ...
    protected void initialize(ConnectionSourceSettings settings) {
        ...
        Iterable<CustomTypeMarshaller> customTypeMarshallers = ConfigurationUtils.findServices(settings.getCustom().getTypes(), CustomTypeMarshaller)
        for (CustomTypeMarshaller customTypeMarshaller in customTypeMarshallers) {
    ...
    abstract MappingFactory getMappingFactory()
    ...
    void configure(PropertyResolver configuration) {
        ...
        Iterable<CustomTypeMarshaller> customTypeMarshallers = ConfigurationUtils.findServices(configuration, configurationKey, CustomTypeMarshaller)
        for (CustomTypeMarshaller customTypeMarshaller in customTypeMarshallers) {
            if (customTypeMarshaller.supports(this)) {
    ...
    ProxyFactory getProxyFactory() {
        if (this.proxyFactory == null) {
            ClassLoader classLoader = AbstractMappingContext.getClassLoader()
    ...
    @PackageScope
    static class DefaultProxyFactoryCreator {
    ...
    Collection<PersistentEntity> addPersistentEntities(Class... javaClasses) {
        ...
        for (Class javaClass in javaClasses) {
        ...
        for (Listener eventListener in eventListeners) {
            for (PersistentEntity entity in entities) {
    ...
    protected boolean isValidMappingStrategy(Class<?> javaClass, Object mappingStrategy) {
        ...
        String suffix = MappingContext.getSimpleName()
    ...
    PersistentEntity getPersistentEntity(String name) {
        ...
        final int proxyIndicator = entityName.indexOf('_$$_')
        if (proxyIndicator > -1) {
            entityName = entityName.substring(0, proxyIndicator)
        }

        return persistentEntitiesByName.get(entityName)
    }
    ...
    EntityAccess createEntityAccess(PersistentEntity entity, Object instance) {
        ...
        Object target = proxyFactory.unwrap(instance)
        if (entity != null) {
            PersistentEntity resolvedEntity = entity
            if (resolvedEntity.getJavaClass() != target.getClass()) {
                // try subclass
                final PersistentEntity subEntity = getPersistentEntity(target.getClass().getName())
                if (subEntity != null) {
                    resolvedEntity = subEntity
                }
            }
            return new FieldEntityAccess(resolvedEntity, target, getConversionService())
        }
        else {
            EntityReflector reflector = FieldEntityAccess.getReflector(target.getClass().getName())
            if (reflector != null) {
                return new FieldEntityAccess(reflector.getPersitentEntity(), target, conversionService)
            }
            else {
                return new BeanEntityAccess(null, target)
            }
        }
    }
