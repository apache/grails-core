...
The `switchMap` transforms an `Observable` and converts the result into another `Observable`. However, this is not the most efficient way to perform updates as you can use https://grails.apache.org/docs/latest/grails-data/hibernate7/manual/index.html#whereQueries[where queries] to update an instance without retrieving it:
