...
// NEW: Forge vs Shell Context
NOTE: Forge commands like `create-app` require terminal execution via `grails -t forge`. They will not run inside the Grails shell (`grails>` prompt), but the shell still supports many other dev commands and tab-completion.
...
// NEW: Clarify that create-app help output comes from the Forge CLI
The `create-app` command is part of the Forge CLI. To view its help information, you must use `./grails -t forge` from the terminal. You cannot access this output from the Grails shell (`grails>` prompt).
// NEW: This command must be run from terminal, not inside the interactive shell
[source,shell]
----
...
// NEW: Must run outside of interactive shell using Forge
[source,shell]
----
