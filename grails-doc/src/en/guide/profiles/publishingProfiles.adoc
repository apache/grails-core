...
To publish a profile using this plugin to Maven Central first upload the source to https://github.com[Github] (closed source profiles will not be accepted). Then register for an account on https://central.sonatype.com[Maven Cnetral] and configure your keys as follows in the profile's `build.gradle` file:
