...
When Spring Boot Developer Tools is on the classpath, Grails bootstraps Hibernate with DevTools' restart class loader. That keeps domain class identity consistent with Hibernate's metamodel without moving GORM or Grails jars onto the restart loader. Without that alignment, GORM calls such as `Role.count()` and `save()` can fail with `IllegalArgumentException: Not an entity` while `get()` and HQL still work.
