...
For this reason, the decision was made to restore publishing of the `grails-shell` in later versions of Grails 6. The other problem is to customize application generation, `grails forge` would need to be forked, customized, published, and hosted some where.
The process of building & hosting a custom version of `grails-forge` was found to be cumbersome for some people because the previous shell allowed for customization by providing a directory that laid out a skeleton of what create.
...
1. `grails-forge-cli` - the forge cli that was shipped as `grails` initially in Grails 6. This CLI has autocompletion and long term we intend to fold the legacy shell functions into it - including the ability to create custom commands & supporting a custom layout.
