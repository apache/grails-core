...
//NEW
NOTE: As of **Grails 7**, most CLI commands must be run using the `-t forge` flag to access the Grails Forge toolchain. For example, `grails create-app` becomes `grails -t forge create-app`. Interactive mode is still available by simply typing `grails` without additional arguments.
