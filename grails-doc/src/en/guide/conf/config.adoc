...
If you prefer to use Grails 2.0-style Groovy configuration then it is possible to specify configuration using Groovy's https://docs.groovy-lang.org/latest/html/documentation/#_configslurper[ConfigSlurper] syntax. Two Groovy configuration files are available: `grails-app/conf/application.groovy` and `grails-app/conf/runtime.groovy`:
