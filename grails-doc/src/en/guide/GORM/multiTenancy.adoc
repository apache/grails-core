...
Two further overloads select which datastore the tenant applies to when an application has more than one:

[source,groovy]
----
// by datastore type
Tenants.withId(HibernateDatastore, 'acme') { Person.count() }

// by the datastore that maps a given domain class
Tenants.withIdForDomain(Person, 'acme') { Person.count() }
----

NOTE: `withId(Class, ...)` takes a *datastore* type; `withIdForDomain(Class, ...)` takes a *domain* class. They
need separate names because both erase to the same signature.
...
`withTenantForDomain(Class, tenantId, closure)` overload picks the datastore by domain class.
