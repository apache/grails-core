...
Grails 7 disabled Groovy invokedynamic for `GroovyCompile` tasks because of performance regressions on Groovy 4 (https://github.com/apache/grails-core/issues/15293[#15293]).
Grails 9 turns invokedynamic back on by default for generated apps (`grails { indy }`) and published framework modules (`CompilePlugin`).
You do not need to set `grails { indy = true }`.
That matches Groovy 6's preferred dispatch. CI can still compile without indy via `-PgrailsIndy=false`.

To keep classic callsite bytecode:
...
Non-indy builds on Groovy 6 need `org.apache.groovy:groovy-callsite` on the classpath. The Grails BOM already manages that coordinate.
