...
GORM for Hibernate is provided by `grails-data-hibernate7` (with `grails-hibernate7-bom`), integrating Hibernate ORM 7.4 - the actively developed line of Hibernate ORM.

NOTE: Earlier Grails releases also offered a Hibernate 5 integration (`grails-data-hibernate5`). It has been removed; applications still on it must migrate to Hibernate 7 before upgrading. See the link:upgrading.html#_hibernate_5_to_hibernate_7_migration[Hibernate 5 to Hibernate 7 Migration] guide for the full list of breaking changes and their workarounds.
