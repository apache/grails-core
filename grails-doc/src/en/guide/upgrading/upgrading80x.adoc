...
Grails 8 keeps GORM's non-breaking behavior for HQL `GString` queries: interpolated values are converted to query parameters instead of being copied into the final HQL text.
Applications may now see a warning when a `GString` query is passed to methods such as `find`, `findAll`, `executeQuery`, or `executeUpdate`.
The warning is informational and does not reject the query.

Prefer explicit named parameters for values that come from requests, files, external services, or other untrusted input:
...
Only build query structure dynamically from validated, trusted names such as a domain property allowlist.
Keep user-controlled values in `namedParams` rather than concatenating them into the HQL string.
