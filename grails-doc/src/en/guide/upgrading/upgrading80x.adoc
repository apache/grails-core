...
Grails 8 introduces a new BOM for projects that combine Micronaut integration with Hibernate 7:
...
The existing `grails-micronaut-bom` remains available. If the framework changes its default Hibernate version in the future, `grails-micronaut-bom` will track the new default while the Hibernate-specific BOM remains pinned to its version.
