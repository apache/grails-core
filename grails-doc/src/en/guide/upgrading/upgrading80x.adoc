...
Grails 8 introduces two new BOMs for projects that combine Micronaut integration with a specific Hibernate version:
...
Both BOMs extend `grails-base-bom` and the Micronaut platform directly, and include the appropriate Hibernate dependency versions alongside the Micronaut-specific overrides (Groovy 5, Spock 2.4, etc.). This ensures no version conflicts when consumed via `enforcedPlatform`.
