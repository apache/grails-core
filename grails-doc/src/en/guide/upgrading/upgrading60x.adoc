...
===== 12.25 JSON Rendering of Java 8 Date/Time Types

JSON rendering of `java.time.Instant` and `java.time.LocalDateTime` has been updated for consistency across both standard JSON converters (`render ... as JSON`) and JSON views (`.gson` files).
...
*LocalDateTime*: Previously rendered without timezone information. Now consistently renders as ISO-8601 format with Z suffix (assuming UTC):
...
If your application or API consumers depend on the previous JSON format for `Instant` or `LocalDateTime` fields:
...
3. **Consistency**: All temporal types now render consistently, making it easier to handle dates uniformly across your application.
