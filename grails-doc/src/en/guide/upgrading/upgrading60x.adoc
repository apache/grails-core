...
JSON rendering of `java.util.Calendar`, `java.time.Instant`, and `java.time.LocalDateTime` has been updated for consistency across both standard JSON converters (`render ... as JSON`) and JSON views (`.gson` files).
...
NOTE: `LocalDateTime` does not include timezone information, so it renders without the Z suffix, unlike `Date`, `Calendar`, and `Instant` which represent specific points in time.

*java.util.Date*: Continues to render as ISO-8601 with Z suffix (unchanged):
...
If your application or API consumers depend on the previous JSON format for `Calendar`, `Instant`, or `LocalDateTime` fields:
...
2. **Date Parsing**: The new format is a standard ISO-8601 string that can be parsed using `Instant.parse()` or `LocalDateTime.parse()` with appropriate formatters.

3. **Consistency**: Temporal types with timezone information (`Date`, `Calendar`, `Instant`) render with Z suffix, while `LocalDateTime` (which has no timezone) renders without it, matching Spring Boot's behavior.
