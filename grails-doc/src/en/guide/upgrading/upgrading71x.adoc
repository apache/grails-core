...
need to tolerate the optional ` (ip: <value>, user: <value>)` suffix. Auditor logging is enabled by default
when an `AuditorAware` bean is registered; remote-address logging is opt-in. Toggle each independently:
...
grails:
    exceptionresolver:
        logAuditor: false       # drop ' user: <value>'
        logRemoteAddr: true     # add ' ip: <value>'
----

When neither applies to a given request — no `AuditorAware` bean registered and the IP suffix not enabled — the
headline collapses to its pre-7.1 form. See the Logging Full Stack Traces section of the user guide for details.
