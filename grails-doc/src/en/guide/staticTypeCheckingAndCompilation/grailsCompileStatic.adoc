...
Rather than annotating each class individually, static compilation can be enabled for every controller, service and tag library in an application. The Grails Gradle plugin exposes three opt-ins in the `grails` build configuration block:

[source,groovy]
.build.gradle
----
grails {
    compileStaticControllers = true   // compile every grails-app/controllers class with @GrailsCompileStatic
    compileStaticServices = true      // compile every grails-app/services class with @GrailsCompileStatic
    compileStaticTagLibs = true       // compile every grails-app/taglib class with @GrailsCompileStatic
}
----

All three options are disabled by default and are independent, so each artefact type can be enabled on its own. To turn them all on at once, use the `compileStaticArtefacts` shortcut instead:

[source,groovy]
.build.gradle
----
grails {
    compileStaticArtefacts = true     // shortcut for controllers + services + tag libraries
}
----
...
[NOTE]
====
For tag libraries, define tags as methods (for example `def myTag(Map attrs, Closure body)`). The deprecated closure-field form (`def myTag = { attrs, body -> }`) cannot be statically compiled when it dispatches to other tags; such a tag library should be migrated to method-based tags or kept dynamic with a per-class `@CompileDynamic`.
====
...
class LegacyController {
    // compiled dynamically even when grails.compileStaticControllers is enabled
}
----
