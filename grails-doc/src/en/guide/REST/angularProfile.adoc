...
//NEW
NOTE: As of **Grails 7**, CLI commands such as `create-app` must include the `-t forge` flag to access the correct Grails Forge toolchain.
For example, the command should be written as:

[source,bash]
----
grails -t forge create-app my-app --profile angular
----

Older versions (Grails 6 and below) do not require this flag.
====
