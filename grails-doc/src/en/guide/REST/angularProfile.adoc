...
[NOTE]
====
As of **Grails 7**, both the Grails Shell CLI and the Forge CLI are available.

To use the Forge CLI for generating a `rest-api` app, include the `-t forge` flag:

[source,bash]
----
grails -t forge create-restapi my-app
----

To use the traditional Grails Shell CLI for profile-based generation (like Angular):

[source,bash]
----
grails create-app --profile angular my-app
----

Use `-t forge` only when accessing Forge-specific generation options. Older versions (Grails 6 and below) do not require this flag.
====
