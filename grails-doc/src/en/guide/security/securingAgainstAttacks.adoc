...
GORM binds interpolated values in `GString` HQL as query parameters, but Grails logs a warning when a `GString` query is passed to methods such as `find`, `findAll`, `executeQuery`, or `executeUpdate`.
Prefer explicit named parameters in application code because they make the safe boundary between query structure and user-controlled values clear.
