...
When a wildcard-captured action matches a real action on the controller, it takes priority over other mappings where the action is hardcoded. This means `$action` mappings that resolve to valid actions will always be preferred over `$id` mappings with a hardcoded action, regardless of URL mapping sort order.
