...
`max-file-size` = The maximum size allowed for an uploaded file.

`max-request-size` = The maximum size allowed for a multipart/form-data request.

The `spring.servlet.multipart` namespace also supports Spring Boot's `location`, `file-size-threshold`, and `enabled` properties.
