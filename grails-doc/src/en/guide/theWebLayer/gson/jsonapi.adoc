...
The JSON API specification has link:https://jsonapi.org/format/#errors[instructions on how to render errors]. In the case of the Grails implementation, this applies to both exceptions and validation errors.
...
The JSON API specification has a section which explains https://jsonapi.org/format/#fetching-pagination[pagination]. The Grails implementation follows that specification as it is written.
...
The JSON API implementation in Grails supports the same includes and excludes support as normal json views. Please see the section on link:gsonRendering[rendering] for details.
