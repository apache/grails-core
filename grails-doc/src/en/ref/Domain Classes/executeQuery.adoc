...
When using Hibernate 7, GORM binds interpolated values in a `GString` HQL query as query parameters, and GORM logs a warning for `GString` queries so applications can migrate toward explicit named parameters.
