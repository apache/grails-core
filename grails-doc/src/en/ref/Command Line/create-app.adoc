...
// NEW: Updated usage line with correct CLI invocation
...
$ cd bookstore
----
// NEW -- The basic create-app bookstore example (without Forge options) should not use -t forge unless it's doing something Forge-specific.
...
// NEW: Removed redundant or legacy duplicate section below
// The second "== Grails Forge create-app" block appears to be a duplicate and is no longer necessary
// because we’ve already clarified that `create-app` is a Forge CLI command and included examples.
