...
* `generator` (optional) - The name of the generator to use. Can be `increment`, `identity`, `sequence`, `seqhilo`, `uuid`, `uuid2`, `assigned`, `select`, `foreign`, `sequence-identity`, `enhanced-sequence` or `enhanced-table`. See {hibernate7Reference}#identifiers-generators[Hibernate reference documentation] for more information.
...
See the section on  link:{hibernate7GuideFromRef}index.html#identity[Custom Database Identity] in the user guide for more information.
