...
NOTE: The key `updateable` (with an extra 'e') is a deprecated alias for `updatable` and will be removed in a future release. A deprecation warning is logged at startup when the old key is used.
