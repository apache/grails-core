...
A domain class fulfills the M in the Model View Controller (MVC) pattern and represents a persistent entity that is mapped onto an underlying database table. In Grails a domain is a class that lives in the `grails-app/domain` directory. A domain class can be created with the link:{commandLineRefFromRef}create-domain-class.html[create-domain-class] command:
...
You can turn on autowire for all your domain classes using the link:{hibernate7GuideFromRef}index.html#_the_default_mapping_constraints[Default Mapping] setting:
