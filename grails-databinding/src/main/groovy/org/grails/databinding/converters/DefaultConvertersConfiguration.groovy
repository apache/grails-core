...
@Configuration
@CompileStatic
class DefaultConvertersConfiguration {
    ...
    DefaultConvertersConfiguration(GrailsApplication grailsApplication, DataBindingConfigurationProperties configurationProperties) {
        ...
        try {
            localResolver = grailsApplication.getMainContext().getBean(LocaleResolver)
    ...
    protected ValueConverter defaultGrailsBigDecimalConverter() {
        ...
        converter.setTargetType(BigDecimal)
    ...
    protected ValueConverter defaultGrailsBigIntegerConverter() {
        LocaleAwareBigDecimalConverter converter = new LocaleAwareBigDecimalConverter()
        converter.setTargetType(BigInteger)
        converter.setLocaleResolver(localResolver)
        return converter
    }
    ...
    protected LocaleAwareNumberConverter integerConverter() {
        final LocaleAwareNumberConverter converter = new LocaleAwareNumberConverter()
        converter.setLocaleResolver(localResolver)
        converter.setTargetType(Integer)
        return converter
    }
    ...
    protected LocaleAwareNumberConverter doubleConverter() {
        final LocaleAwareNumberConverter converter = new LocaleAwareNumberConverter()
        converter.setLocaleResolver(localResolver)
        converter.setTargetType(Double)
        return converter
    }
