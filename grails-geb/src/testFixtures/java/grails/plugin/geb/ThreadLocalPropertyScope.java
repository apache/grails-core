 ...
 * <p>
 * Property lookup must stay in Java (#16157): Groovy 6 reads a system property while
 * dispatching a method call, so a lookup backed by a Groovy closure, as the previous
 * Groovy implementation's {@code ThreadLocal.withInitial { [:] }} was, re-entered itself
 * until a {@code StackOverflowError} killed the Gradle test worker's dispatcher and left
 * the worker and the daemon waiting on each other. Permanent. App test build: a Grails
 * application's Geb tests use this fixture.
 */
