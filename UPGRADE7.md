...
- Command line changes:
  - The `package-plugin` command has been removed since Grails 4.0.4
    - For packaging plugins, use `./gradlew assemble` instead
    - For building plugin JARs, use `./gradlew jar`
    - This aligns with Gradle's standard build lifecycle tasks
    - Previous documentation references to `grails package-plugin` should be updated to use these Gradle tasks
