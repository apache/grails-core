 ...
 */
@CompileStatic
class JSON extends AbstractConverter<JSONWriter> implements IncludeExcludeConverter<JSONWriter> {

    private static final Log log = LogFactory.getLog(JSON)
    ...
    protected ConverterConfiguration<JSON> initConfig() {
        return ConvertersConfigurationHolder.getConverterConfiguration(JSON)
    }
    ...
    JSONWriter getWriter() throws ConverterException {
        return writer
    }
    ...
    void property(String key, Object value) throws JSONException, ConverterException {
        writer.key(key)
        this.value(value)
    }
     ...
     */
    String toString(boolean prettyPrint) throws JSONException {
     ...
     */
    static JSONElement parse(Reader reader) throws ConverterException {
     ...
     * @throws ConverterException when the JSON content is not valid
     */
    static JSONElement parse(String source) throws ConverterException {
        try {
     ...
     * @throws ConverterException when the JSON content is not valid
     */
    static JSONElement parse(InputStream is, String encoding) throws ConverterException {
    ...
    protected void handleCircularRelationship(Object o) throws ConverterException {
        switch (circularReferenceBehaviour) {
            case CircularReferenceBehaviour.DEFAULT:
                final boolean isCollection = Collection.isAssignableFrom(o.getClass())
                final boolean isMap = Map.isAssignableFrom(o.getClass())
            ...
            case CircularReferenceBehaviour.INSERT_NULL:
                value(null)
                break
            case CircularReferenceBehaviour.PATH:
                Map<String, Object> props = new HashMap<>()
                props.put('class', o.getClass())
                int idx = referenceStack.indexOf(o)
                ...
                value(props)
                break
            case CircularReferenceBehaviour.IGNORE:
                break
        }
    }

    static ConverterConfiguration<JSON> getNamedConfig(String configName) throws ConverterException {
        ConverterConfiguration<JSON> cfg = ConvertersConfigurationHolder.getNamedConverterConfiguration(configName, JSON)
        if (cfg == null) {
    ...
    static Object use(String configName, Closure<?> callable) throws ConverterException {
        ...
        ConvertersConfigurationHolder.setThreadLocalConverterConfiguration(JSON, cfg)
        try {
    ...
    static void use(String cfgName) throws ConverterException {
        if (cfgName == null || 'default' == cfgName) {
            ...
            ConvertersConfigurationHolder.setThreadLocalConverterConfiguration(JSON, getNamedConfig(cfgName))
        }
    }
    ...
    static void registerObjectMarshaller(ObjectMarshaller<JSON> om) throws ConverterException {
        ConverterConfiguration<JSON> cfg = ConvertersConfigurationHolder.getConverterConfiguration(JSON)
        if (cfg == null) {
            throw new ConverterException('Default Configuration not found for class ' + JSON.name)
        }
        if (!(cfg instanceof DefaultConverterConfiguration<?>)) {
            cfg = new DefaultConverterConfiguration<>(cfg)
            ConvertersConfigurationHolder.setDefaultConfiguration(JSON, cfg)
        }
        ((DefaultConverterConfiguration<JSON>) cfg).registerObjectMarshaller(om)
    }

    static void registerObjectMarshaller(ObjectMarshaller<JSON> om, int priority) throws ConverterException {
        ConverterConfiguration<JSON> cfg = ConvertersConfigurationHolder.getConverterConfiguration(JSON)
        if (cfg == null) {
            throw new ConverterException('Default Configuration not found for class ' + JSON.name)
        }
    ...
    static void createNamedConfig(String name, Closure<?> callable) throws ConverterException {
        DefaultConverterConfiguration<JSON> cfg = new DefaultConverterConfiguration<>(ConvertersConfigurationHolder.getConverterConfiguration(JSON))
        try {
            callable.call(cfg)
            ConvertersConfigurationHolder.setNamedConverterConfiguration(JSON, name, cfg)
        }
        catch (Exception e) {
    ...
    class Builder extends BuilderSupport {
        ...
        void execute(Closure<?> callable) {
            ...
            invokeMethod('json', [callable] as Object[])
        }
        ...
        protected Object createNode(Object key, Map valueMap) {
            try {
                if (stack.peek() == BuilderMode.OBJECT) {
