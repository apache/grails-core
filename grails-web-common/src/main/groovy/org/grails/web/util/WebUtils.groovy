...
class WebUtils extends org.springframework.web.util.WebUtils {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(WebUtils)
    ...
    public static final String MULTIPART_HTTP_SERVLET_REQUEST_ATTRIBUTE = MultipartHttpServletRequest.name
    ...
    static ViewResolver lookupViewResolver(ApplicationContext wac) {
        final CompositeViewResolver viewResolver = wac.getBean(CompositeViewResolver.BEAN_NAME, CompositeViewResolver)
    ...
    static HandlerInterceptor[] lookupHandlerInterceptors(ServletContext servletContext) {
        ...
        for (WebRequestInterceptor webRequestInterceptor in webRequestInterceptors) {
            allHandlerInterceptors.add(new WebRequestHandlerInterceptorAdapter(webRequestInterceptor))
        }
        final Collection<HandlerInterceptor> handlerInterceptors = wac.getBeansOfType(HandlerInterceptor).values()
    ...
    static WebRequestInterceptor[] lookupWebRequestInterceptors(ServletContext servletContext) {
        ...
        final Collection<WebRequestInterceptor> webRequestInterceptors = wac.getBeansOfType(WebRequestInterceptor).values()
     ...
     * @return The GrailsApplication instance
     */
    static GrailsApplication findApplication(ServletContext servletContext) {
     ...
     */
    static ApplicationContext findApplicationContext(ServletContext servletContext) {
     ...
     */
    static View resolveView(HttpServletRequest request, String viewName, String controllerName, ViewResolver viewResolver) throws Exception {
     ...
     */
    @Deprecated
    static String addViewPrefix(String viewName) {
        GrailsWebRequest webRequest = GrailsWebRequest.lookup()
        return addViewPrefix(viewName, webRequest != null ? webRequest.getControllerName() : null)
    }

    static String addViewPrefix(String viewName, String controllerName) {
    ...
    static Map<String, Object> exposeRequestAttributesAndReturnOldValues(HttpServletRequest request, Map<String, ?> attributes) {
    ...
    static void cleanupIncludeRequestAttributes(HttpServletRequest request, Map<String, Object> toRestore) {
    ...
    static void exposeIncludeRequestAttributes(HttpServletRequest request) {
    ...
    static Map<String, Object> fromQueryString(String queryString) {
     ...
     * @return The format or null if none
     */
    static String getFormatFromURI(String uri, MimeType[] mimeTypes) {
        if (uri.endsWith('/')) {
            return null
        }
    ...
    static boolean areFileExtensionsEnabled() {
        ...
        return config.getProperty(ENABLE_FILE_EXTENSIONS, Boolean, true)
    }
     ...
     */
    static boolean isForward(HttpServletRequest request) {
     ...
     * @return True if it is a forward request
     */
    static boolean isAsync(HttpServletRequest request) {
     ...
     * @return True if it is a forward request
     */
    static boolean isError(HttpServletRequest request) {
     ...
     */
    static boolean isInclude(HttpServletRequest request) {
     ...
     */
    static boolean isForwardOrInclude(HttpServletRequest request) {
     ...
     */
    static MultipartHttpServletRequest resolveMultipartRequest(HttpServletRequest request) {
        MultipartHttpServletRequest resolved = getNativeRequest(request, MultipartHttpServletRequest)
        if (resolved != null) {
            return resolved
        }
        Object attribute = request.getAttribute(MULTIPART_HTTP_SERVLET_REQUEST_ATTRIBUTE)
        attribute instanceof MultipartHttpServletRequest ? (MultipartHttpServletRequest) attribute : null
    }
     ...
     */
    static boolean isMultipartContentType(HttpServletRequest request) {
     ...
     */
    static Map<String, String[]> readParameterMap(HttpServletRequest request) {
        readTolerantly(request, { request.getParameterMap() } as Supplier<Map<String, String[]>>, Collections.<String, String[]> emptyMap())
    }
     ...
     * @see #readTolerantly(HttpServletRequest, Supplier, Object)
     */
    static String readParameter(HttpServletRequest request, String name) {
        readTolerantly(request, { request.getParameter(name) } as Supplier<String>, null)
    }
     ...
     * @see #readTolerantly(HttpServletRequest, Supplier, Object)
     */
    static Enumeration<String> readParameterNames(HttpServletRequest request) {
        readTolerantly(request, { request.getParameterNames() } as Supplier<Enumeration<String>>, Collections.<String> emptyEnumeration())
    }
