 ...
 */
@CompileStatic
class GroovyPagesUriSupport implements GroovyPagesUriService {
    ...
    String getTemplateURI(String controllerName, String templateName, boolean includeExtension) {
        ...
        int lastSlash = templateName.lastIndexOf((int) SLASH)
        if (lastSlash > -1) {
    ...
    String getAbsoluteTemplateURI(String templateName, boolean includeExtension) {
        ...
        if (tmp.indexOf((int) SLASH) > -1) {
            buf.append(SLASH)
            int i = tmp.lastIndexOf((int) SLASH)
    ...
    private String getAbsoluteViewURIInternal(String viewName, FastStringWriter buf, boolean includeSuffix) {
        ...
        if (tmp.indexOf((int) SLASH) > -1) {
            buf.append(SLASH)
            buf.append(tmp.substring(0, tmp.lastIndexOf((int) SLASH)))
            buf.append(SLASH)
            buf.append(tmp.substring(tmp.lastIndexOf((int) SLASH) + 1, tmp.length()))
        }
        else {
            buf.append(SLASH)
