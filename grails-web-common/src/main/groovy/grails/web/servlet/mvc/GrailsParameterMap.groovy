...
@CompileStatic
class GrailsParameterMap extends TypeConvertingMap implements Cloneable {
    ...
    GrailsParameterMap(HttpServletRequest request) {
        ...
        Map<String, Object> requestMap = (Map) WebUtils.readParameterMap(request)
        ...
        if (multipartRequest != null) {
            ...
            if (mergeParameters || !fileMap.isEmpty()) {
                requestMap = (Map) new LinkedHashMap(requestMap)
            }
            if (mergeParameters) {
                for (Map.Entry<String, String[]> entry in multipartParameters.entrySet()) {
                    requestMap.putIfAbsent(entry.getKey(), entry.getValue())
                }
            }
            for (Map.Entry<String, List<MultipartFile>> entry in fileMap.entrySet()) {
    ...
    Object clone() {
        if (this.@wrappedMap.isEmpty()) {
            ...
            Map clonedMap = new LinkedHashMap(this.@wrappedMap)
    ...
    Object get(Object key) {
        ...
        if (nestedDateMap.containsKey(key)) {
            ...
            if (returnValue instanceof String[]) {
                ...
                return DefaultGroovyMethods.subMap(this.@wrappedMap, (Collection) key)
            }
        }
        return returnValue
    }
    ...
    Object put(Object key, Object value) {
        ...
        if (key instanceof String) {
            ...
            if (keyString.indexOf('.') > -1) {
                processNestedKeys(this, keyString, keyString, this.@wrappedMap)
            }
        }
        return returnValue
    }
    ...
    @Override
    void putAll(Map map) {
        for (Object entryObj in map.entrySet()) {
    ...
    Date getDate(String name) {
        Object returnValue = this.@wrappedMap.get(name)
    ...
    private Date lazyEvaluateDateParam(Object key) {
        ...
        for (Object entryObj in entrySet()) {
            ...
            if (entryKey instanceof String) {
                ...
                final String prefix = key.toString() + '_'
        ...
        try {
            return (Date) editor.assemble(Date, dateParams)
        }
