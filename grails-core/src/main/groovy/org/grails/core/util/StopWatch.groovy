...
class StopWatch {
     ...
     * @see #stop()
     */
    void start(String taskName) throws IllegalStateException {
     ...
     */
    long getLastTaskTimeMillis() throws IllegalStateException {
     ...
     */
    String getLastTaskName() throws IllegalStateException {
        if (this.lastTaskInfo == null) {
     ...
     */
    TaskInfo getLastTaskInfo() throws IllegalStateException {
        if (this.lastTaskInfo == null) {
    ...
    String prettyPrint() {
        ...
        Arrays.sort(taskInfos, new Comparator<TaskInfo>() {
            @Override
            int compare(TaskInfo o1, TaskInfo o2) {
                return Long.compare(o1.getTimeMillis(), o2.getTimeMillis())
            }
        })
        for (TaskInfo task in taskInfos) {
    ...
    String toString() {
        StringBuilder sb = new StringBuilder(shortSummary())
        for (TaskInfo task in getTaskInfo()) {
