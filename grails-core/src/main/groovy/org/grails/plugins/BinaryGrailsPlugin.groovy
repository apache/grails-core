...
@CompileStatic
class BinaryGrailsPlugin extends DefaultGrailsPlugin {
     ...
     */
    BinaryGrailsPlugin(Class<?> pluginClass, PluginDescriptor descriptor, GrailsApplication application) {
    ...
    protected void initializeViewMap(PluginDescriptor descriptor) {
        ...
        try {
            ...
            for (Object view in viewsProperties.keySet()) {
     ...
     */
    Properties getProperties(final Locale locale) {
    ...
    private Resource[] filterResources(Resource[] resources, Locale locale) {
        ...
        for (Resource resource in resources) {
            ...
            if (fn.indexOf((int) UNDERSCORE) > -1) {
                if (fn.endsWith(String.valueOf(UNDERSCORE) + locale.toString() + PROPERTIES_EXTENSION)) {
                    finalResources.add(resource)
                }
                else if (fn.endsWith(String.valueOf(UNDERSCORE) + locale.getLanguage() + UNDERSCORE + locale.getCountry() + PROPERTIES_EXTENSION)) {
                    finalResources.add(resource)
                }
                else if (fn.endsWith(String.valueOf(UNDERSCORE) + locale.getLanguage() + PROPERTIES_EXTENSION)) {
                    finalResources.add(resource)
                }
            }
            else {
    ...
    private void loadFromResources(Properties properties, Resource[] resources) throws IOException {
        for (Resource messageResource in resources) {
     ...
     */
    Class resolveView(String viewName) {
