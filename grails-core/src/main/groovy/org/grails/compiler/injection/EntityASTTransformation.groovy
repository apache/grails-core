...
@CompileStatic
class EntityASTTransformation implements ASTTransformation, CompilationUnitAware, TransformWithPriority {
    ...
    void applyTransformation(SourceUnit sourceUnit, ClassNode classNode) {
        if (GrailsASTUtils.isApplied(classNode, EntityASTTransformation)) {
            return
        }
        GrailsASTUtils.markApplied(classNode, EntityASTTransformation)
        ...
        for (ClassInjector injector in domainInjectors) {
            try {
