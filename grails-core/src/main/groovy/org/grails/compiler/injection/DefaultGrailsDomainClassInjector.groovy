...
@AstTransformer
@CompileStatic
class DefaultGrailsDomainClassInjector implements GrailsDomainClassInjector, GrailsArtefactClassInjector {
    ...
    private Collection<PropertyNode> createPropertiesForBelongsToOrHasOneExpression(Expression e, ClassNode classNode) {
        ...
        if (e instanceof MapExpression) {
            ...
            for (MapEntryExpression mme in me.getMapEntryExpressions()) {
    ...
    private void injectAssociationProperties(ClassNode classNode, List<PropertyNode> propertiesToAdd) {
        for (PropertyNode pn in propertiesToAdd) {
    ...
    private void injectToStringMethod(ClassNode classNode) {
        ...
        final boolean hasToStringAnnotation = GrailsASTUtils.hasAnnotation(classNode, groovy.transform.ToString)
