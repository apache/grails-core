...
final class I18nDescriptors {
     ...
     */
    static List<I18nDescriptor> load(ClassLoader classLoader) {
    ...
    private static I18nDescriptor read(URL url) throws IOException {
        ...
        if (!SUPPORTED_FORMAT_VERSION.equals(formatVersion)) {
            throw new IllegalStateException("Unsupported i18n descriptor format version '${formatVersion}' at " +
                    "${url}. This Grails version understands version ${SUPPORTED_FORMAT_VERSION}; the artifact " +
                    'was built by a newer Grails Gradle plugin.')
        }
        ...
        if (!I18nDescriptor.TYPE_APPLICATION.equals(type) && !I18nDescriptor.TYPE_PLUGIN.equals(type)) {
            throw new IllegalStateException("Invalid ${ARTIFACT_TYPE} '${type}' at ${url}. Expected " +
                    "'${I18nDescriptor.TYPE_APPLICATION}' or '${I18nDescriptor.TYPE_PLUGIN}'.")
        }
    ...
    private static void rejectAmbiguousClasspath(List<I18nDescriptor> descriptors) {
        ...
        for (I18nDescriptor descriptor : descriptors) {
            if (descriptor.isApplication()) {
                applications.add(descriptor.name())
            }
        }
        ...
        for (I18nDescriptor descriptor : descriptors) {
            if (!descriptor.isApplication()) {
                String normalisedName = PluginUtils.normalizePluginName(descriptor.name())
                List<String> names = pluginsByNormalisedName.get(normalisedName)
                if (names == null) {
                    names = new ArrayList<>()
                    pluginsByNormalisedName.put(normalisedName, names)
                }
                names.add(descriptor.name())
            }
        }
        List<String> duplicates = []
        for (List<String> names : pluginsByNormalisedName.values()) {
            if (names.size() > 1) {
                duplicates.add(String.join(' and ', names))
            }
        }
