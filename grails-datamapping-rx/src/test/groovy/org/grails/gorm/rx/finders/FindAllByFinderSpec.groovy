...
class FindAllByFinderSpec extends Specification {
    ...
    def "obtains its mapping context from the datastore client when constructed"() {
        when:
        def finder = new FindAllByFinder(datastoreClient)
    ...
    def "finds all without arguments by invoking findAll on the RX query"() {
        given:
        def finder = new FindAllByFinder(datastoreClient)
        def titleExpression = new MethodExpression.Equal(Book, 'title')
        titleExpression.setArguments(['Shogun'] as Object[])
        def invocation = new DynamicFinderInvocation(Book, 'findAllByTitle', [] as Object[], [titleExpression], null, null)
    ...
    def "finds all with map arguments by invoking findAll(Map) on the RX query and applying the arguments"() {
        given:
        def finder = new FindAllByFinder(datastoreClient)
        def invocation = new DynamicFinderInvocation(Book, 'findAllByTitle', [[max: 5]] as Object[], [], null, null)
        def observable = Observable.just(new Book(title: 'Shogun'))
    ...
    def "applies detached criteria to the query when present"() {
        given:
        def finder = new FindAllByFinder(datastoreClient)
        def invocation = new DynamicFinderInvocation(Book, 'findAllByTitle', [] as Object[], [], null, null)
    ...
    def "applies additional criteria to the query when present"() {
        given:
        def finder = new FindAllByFinder(datastoreClient)
        def invocation = new DynamicFinderInvocation(Book, 'findAllByTitle', [] as Object[], [], { -> }, null)
