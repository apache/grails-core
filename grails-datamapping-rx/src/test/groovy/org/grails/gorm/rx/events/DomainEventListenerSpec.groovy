/*
 *  Licensed to the Apache Software Foundation (ASF) under one
 *  or more contributor license agreements.  See the NOTICE file
 *  distributed with this work for additional information
 *  regarding copyright ownership.  The ASF licenses this file
 *  to you under the Apache License, Version 2.0 (the
 *  "License"); you may not use this file except in compliance
 *  with the License.  You may obtain a copy of the License at
 *
 *    https://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing,
 *  software distributed under the License is distributed on an
 *  "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 *  KIND, either express or implied.  See the License for the
 *  specific language governing permissions and limitations
 *  under the License.
 */

package org.grails.gorm.rx.events

import org.grails.datastore.mapping.core.connections.ConnectionSource
import org.grails.datastore.mapping.core.connections.ConnectionSourceSettings
import org.grails.datastore.mapping.core.connections.ConnectionSources
import org.grails.datastore.mapping.engine.EntityAccess
import org.grails.datastore.mapping.engine.event.PreInsertEvent
import org.grails.datastore.mapping.model.MappingContext
import org.grails.datastore.mapping.model.PersistentEntity
import org.grails.datastore.rx.RxDatastoreClient
import spock.lang.Specification

class DomainEventListenerSpec extends Specification {

    static class HookedDomainInstance {
        boolean beforeInsertCalled = false

        void beforeInsert() {
            beforeInsertCalled = true
        }
    }

    RxDatastoreClient buildDatastoreClient(MappingContext mappingContext) {
        ConnectionSource connectionSource = Stub(ConnectionSource) {
            getSettings() >> new ConnectionSourceSettings(autowire: false)
        }
        ConnectionSources connectionSources = Stub(ConnectionSources) {
            getDefaultConnectionSource() >> connectionSource
        }
        Stub(RxDatastoreClient) {
            getMappingContext() >> mappingContext
            getConnectionSources() >> connectionSources
        }
    }

    void "registers itself with the mapping context on construction"() {
        given:
        MappingContext mappingContext = Mock(MappingContext) {
            getPersistentEntities() >> []
        }
        RxDatastoreClient datastoreClient = buildDatastoreClient(mappingContext)

        when:
        new DomainEventListener(datastoreClient)

        then:
        1 * mappingContext.addMappingContextListener(_)
    }

    void "invokes the beforeInsert hook when the event source is the owning datastore client"() {
        given:
        MappingContext mappingContext = Stub(MappingContext) {
            getPersistentEntities() >> []
        }
        RxDatastoreClient datastoreClient = buildDatastoreClient(mappingContext)
        DomainEventListener listener = new DomainEventListener(datastoreClient)
        PersistentEntity entity = Stub(PersistentEntity) {
            getJavaClass() >> HookedDomainInstance
            isVersioned() >> false
        }
        listener.persistentEntityAdded(entity)
        HookedDomainInstance domainInstance = new HookedDomainInstance()
        EntityAccess entityAccess = Stub(EntityAccess) {
            getEntity() >> domainInstance
        }

        when:
        listener.onApplicationEvent(new PreInsertEvent(datastoreClient, entity, entityAccess))

        then:
        domainInstance.beforeInsertCalled
    }

    void "ignores events whose source is not the owning datastore client"() {
        given:
        MappingContext mappingContext = Stub(MappingContext) {
            getPersistentEntities() >> []
        }
        RxDatastoreClient datastoreClient = buildDatastoreClient(mappingContext)
        DomainEventListener listener = new DomainEventListener(datastoreClient)
        PersistentEntity entity = Stub(PersistentEntity) {
            getJavaClass() >> HookedDomainInstance
            isVersioned() >> false
        }
        listener.persistentEntityAdded(entity)
        HookedDomainInstance domainInstance = new HookedDomainInstance()
        EntityAccess entityAccess = Stub(EntityAccess) {
            getEntity() >> domainInstance
        }

        when:
        listener.onApplicationEvent(new PreInsertEvent(new Object(), entity, entityAccess))

        then:
        !domainInstance.beforeInsertCalled
    }

    void "supports the source type matching the owning datastore client's class"() {
        given:
        MappingContext mappingContext = Stub(MappingContext) {
            getPersistentEntities() >> []
        }
        RxDatastoreClient datastoreClient = buildDatastoreClient(mappingContext)
        DomainEventListener listener = new DomainEventListener(datastoreClient)

        expect:
        listener.supportsSourceType(datastoreClient.getClass())
        !listener.supportsSourceType(String)
    }
}
