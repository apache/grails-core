...
class GspAutoConfiguration {
    ...
    @Configuration
    @Import([TagLibraryLookupRegistrar, ReplaceViewResolverRegistrar])
    protected static class GspTemplateEngineAutoConfiguration extends AbstractGspConfig {
        ...
        GrailsConventionGroovyPageLocator groovyPageLocator() {
            ...
            CachingGrailsConventionGroovyPageLocator pageLocator = new CachingGrailsConventionGroovyPageLocator() {
                protected List<String> resolveSearchPaths(String uri) {
                    ...
                    for (String rootPath in templateRootsCleaned) {
        ...
        protected List<String> resolveTemplateRoots() {
            if (templateRoots.length > 0) {
                ...
                for (String rootPath in templateRoots) {
    ...
    protected static class GspViewResolverConfiguration extends AbstractGspConfig {
        ...
        ViewResolver gspViewResolver(GroovyPagesTemplateEngine groovyPagesTemplateEngine, GrailsConventionGroovyPageLocator groovyPageLocator) {
    ...
    protected static class TagLibraryLookupRegistrar implements ImportBeanDefinitionRegistrar {
        ...
        public static final Class<?>[] DEFAULT_TAGLIB_CLASSES = new Class<?>[] { RenderTagLib, RenderSitemeshTagLib }

        @Override
        void registerBeanDefinitions(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry registry) {
            if (!registry.containsBeanDefinition('gspTagLibraryLookup')) {
                GenericBeanDefinition beanDefinition = createBeanDefinition(StandaloneTagLibraryLookup)
        ...
        protected void registerTagLibs(ManagedList<BeanDefinition> list) {
            for (Class<?> taglibClazz in DEFAULT_TAGLIB_CLASSES) {
    ...
    protected static class ReplaceViewResolverRegistrar implements ImportBeanDefinitionRegistrar, EnvironmentAware {
        ...
        @Override
        void setEnvironment(Environment environment) {
            replaceViewResolverBean = environment.getProperty('spring.gsp.replaceViewResolverBean', Boolean, true)
        }
    }

    @ConditionalOnClass([TagLibraryResolverImpl])
    @Configuration
    protected static class GspJspIntegrationConfiguration implements EnvironmentAware {
        @Bean
        TagLibraryResolverImpl jspTagLibraryResolver() {
        ...
        void setEnvironment(Environment environment) {
            if (environment instanceof ConfigurableEnvironment) {
                ...
                configEnv.getPropertySources().addLast(new PropertiesPropertySource(GspJspIntegrationConfiguration.getName(), defaultProperties))
            }
        }
