 ...
 */
@CompileStatic
class GroovyPageParser implements Tokens {

    private static final Logger LOG = LoggerFactory.getLogger(GroovyPageParser)
    ...
    public static final List<String> DEFAULT_TAGLIB_NAMESPACES = Collections.unmodifiableList(Arrays.asList(['g', 'tmpl', 'f', 'asset', 'plugin'] as String[]))
     ...
     */
    private final Deque<Set<String>> typedSetScopes = new ArrayDeque<Set<String>>(
            Collections.singletonList(new LinkedHashSet<String>()))
    ...
    Map<String, String> getJspTags() {
        return jspTags
    }

    void setKeepGeneratedDirectory(File keepGeneratedDirectory) {
        this.keepGeneratedDirectory = keepGeneratedDirectory
    }

    void setEnableGrailsLayoutProcessing(boolean enableGrailsLayoutProcessing) {
    ...
    GroovyPageParser(String name, String uri, String filename, InputStream in, String encoding, String expressionCodecName, ConfigMap configMap) throws IOException {
        this(name, uri, filename, readStream(in, encoding), expressionCodecName, configMap)
    }

    GroovyPageParser(String name, String uri, String filename, String gspSource, ConfigMap configMap) throws IOException {
        this(name, uri, filename, gspSource, null, configMap)
    }

    GroovyPageParser(String name, String uri, String filename, InputStream in, ConfigMap configMap) throws IOException {
        this(name, uri, filename, in, null, null, configMap)
    }

    GroovyPageParser(String name, String uri, String filename, String gspSource, String expressionCodecName, ConfigMap configMap) throws IOException {
    ...
    private void configure(ConfigMap config) {
        compileStaticMode = config.getProperty(GroovyPageParser.CONFIG_PROPERTY_GSP_COMPILESTATIC, Boolean)
        compileStaticStrict = config.getProperty(CONFIG_PROPERTY_GSP_COMPILESTATIC_STRICT, Boolean, Boolean.FALSE)

        Object allowedTagLibsConfigValue = config.getProperty(CONFIG_PROPERTY_GSP_ALLOWED_TAGLIB_NAMESPACES, Object)
        if (allowedTagLibsConfigValue instanceof Iterable) {
            for (Object val in ((Iterable) allowedTagLibsConfigValue)) {
        ...
        setEnableGrailsLayoutProcessing(
                config.getProperty(GroovyPageParser.CONFIG_PROPERTY_GSP_GRAILS_LAYOUT_PREPROCESS, Boolean, true)
        )

        setExpressionCodecDirectiveValue(
                config.getProperty(OutputEncodingSettings.CONFIG_PROPERTY_GSP_CODECS + '.' + OutputEncodingSettings.EXPRESSION_CODEC_NAME, String,
                        config.getProperty(OutputEncodingSettings.CONFIG_PROPERTY_DEFAULT_CODEC, String, OutputEncodingSettings.getDefaultValue(OutputEncodingSettings.EXPRESSION_CODEC_NAME)))
        )

        setStaticCodecDirectiveValue(
                config.getProperty(OutputEncodingSettings.CONFIG_PROPERTY_GSP_CODECS + '.' + OutputEncodingSettings.STATIC_CODEC_NAME, String, OutputEncodingSettings.getDefaultValue(OutputEncodingSettings.STATIC_CODEC_NAME))
        )

        setTaglibCodecDirectiveValue(
                config.getProperty(OutputEncodingSettings.CONFIG_PROPERTY_GSP_CODECS + '.' + OutputEncodingSettings.TAGLIB_CODEC_NAME, String, OutputEncodingSettings.getDefaultValue(OutputEncodingSettings.TAGLIB_CODEC_NAME))
        )

        setOutCodecDirectiveValue(
                config.getProperty(OutputEncodingSettings.CONFIG_PROPERTY_GSP_CODECS + '.' + OutputEncodingSettings.OUT_CODEC_NAME, String, OutputEncodingSettings.getDefaultValue(OutputEncodingSettings.OUT_CODEC_NAME))
        )

        Object keepDirObj = config.getProperty(GroovyPageParser.CONFIG_PROPERTY_GSP_KEEPGENERATED_DIR, Object)
    ...
    void writeHtmlParts(File filename) throws IOException {
    ...
    void writeLineNumbers(File filename) throws IOException {
        DataOutputStream dataOut = null
        try {
            dataOut = new DataOutputStream(new BufferedOutputStream(
                    new FileOutputStream(filename)))
    ...
    String getExpressionText(String text, boolean _safeDereference) {
    ...
    private void makeName(String uri) {
        ...
        for (int ix = 0, ixz = name.length(); ix < ixz; ix++) {
            ...
            if (c < (char) '0' || (c > (char) '9' && c < (char) '@') || (c > (char) 'Z' && c < (char) '_') ||
                    (c > (char) '_' && c < (char) 'a') || c > (char) 'z') {
                c = '_'
            } else if (ix == 0 && c >= (char) '0' && c <= (char) '9') {
                c = '_'
            }
            buf.append(c)
        }
    ...
    private void page() {
        ...
        if (finalPass) {
            ...
            if (isCompileStaticMode()) {
                out.println("@groovy.transform.CompileStatic(extensions = ['" + GroovyPageTypeCheckingExtension.getName() + "'])")
                if (allowedTaglibNamespaces != null && !allowedTaglibNamespaces.isEmpty()) {
                    StringBuilder config = new StringBuilder('@')
                            .append(GroovyPageTypeCheckingConfig.getName())
                            .append("(taglibs = ['")
    ...
    private Set<String> declaredModelNames() {
        ...
        for (String declaration in modelDirectiveValue.split('[;\n]')) {
    ...
    private String resolveGspSuperClassName() {
        Class<?> gspSuperClass = isCompileStaticMode() ? CompileStaticGroovyPage : (isModelRecordingModeEnabled() ? ModelRecordingGroovyPage : GroovyPage)
    ...
    private boolean declareTypedSetVariable(String var) {
        for (Set<String> scope in typedSetScopes) {
    ...
    private void startTag() {
        ...
        if (GroovyPage.DEFAULT_NAMESPACE.equals(ns) && tagRegistry.isSyntaxTag(tagName)) {
            if (tagContext == null) {
                ...
                tagContext.put(GroovyPageParser, this)
            }
     ...
     * @since 8.0.0
     */
    static String checksumOf(byte[] source) {
        try {
    ...
    String getExpressionCodecDirectiveValue() {
    ...
    String getOutCodecDirectiveValue() {
        return outCodecDirectiveValue
    }

    String getStaticCodecDirectiveValue() {
        return staticCodecDirectiveValue
    }

    String getTaglibCodecDirectiveValue() {
        return taglibCodecDirectiveValue
    }

    void setTaglibCodecDirectiveValue(String taglibCodecDirectiveValue) {
        this.taglibCodecDirectiveValue = taglibCodecDirectiveValue
    }

    void setExpressionCodecDirectiveValue(String expressionCodecDirectiveValue) {
        this.expressionCodecDirectiveValue = expressionCodecDirectiveValue
    }

    void setOutCodecDirectiveValue(String outCodecDirectiveValue) {
        this.outCodecDirectiveValue = outCodecDirectiveValue
    }

    void setStaticCodecDirectiveValue(String staticCodecDirectiveValue) {
