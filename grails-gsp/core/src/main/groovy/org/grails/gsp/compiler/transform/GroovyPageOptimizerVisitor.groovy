...
class GroovyPageOptimizerVisitor extends CodeVisitorSupport {
    ...
    private static final ClassNode groovyPageClassNode = new ClassNode(GroovyPage)
    private static final MethodNode writerMethodNode = new ClassNode(GrailsPrintWriter).getMethod('print',
            [new Parameter(new ClassNode(Object), 'obj')] as Parameter[])
    ...
    GroovyPageOptimizerVisitor(ClassNode targetGroovyPage) {
        ...
        thisObjectMethodCall.setMethodTarget(new ClassNode(Closure).getMethods('getThisObject').get(0))
    ...
    */
    ...
    private void proceedCallFromGroovyPageClass(MethodCallExpression call) {
        ...
        if (methodNodeList.size() == 1) {
            ...
            for (MethodNode methodNode in methodNodeList) {
