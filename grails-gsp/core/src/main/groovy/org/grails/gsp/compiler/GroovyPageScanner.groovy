...
class GroovyPageScanner implements Tokens {
    ...
    int nextToken() {
        for (;;) {
            ...
            char c1 = left > 1 ? text.charAt(end1) : (char) 0
            char c2 = left > 2 ? text.charAt(end1 + 1) : (char) 0
    ...
    int getMaxHtmlLength() {
        return maxHtmlLength
    }

    void setMaxHtmlLength(int maxHtmlLength) {
