 ...
 */
@CompileStatic
class GroovyPagesTemplateEngine extends ResourceAwareTemplateEngine implements ResourceLoaderAware, ApplicationContextAware, InitializingBean, BeanClassLoaderAware {
    ...
    private static final Log LOG = LogFactory.getLog(GroovyPagesTemplateEngine)
    ...
    private static class GroovyPagesTemplateEngineCallable implements Callable<CacheEntry<GroovyPageMetaInfo>> {
        ...
        @Override
        CacheEntry<GroovyPageMetaInfo> call() throws Exception {
            return cacheEntry
        }

    }

    void setGroovyPageSourceDecorators(List<GroovyPageSourceDecorator> groovyPageSourceDecorators) {
        this.groovyPageSourceDecorators = groovyPageSourceDecorators
    }

    List<GroovyPageSourceDecorator> getGroovyPageSourceDecorators() {
    ...
    void afterPropertiesSet() {
        ...
        } else if (!classLoader.getClass().equals(GroovyPageClassLoader)) {
     ...
     */
    int[] calculateLineNumbersForPage(String url) {
        try {
    ...
    int mapStackLineNumber(String url, int lineNumber) {
     ...
     */
    @Override
    Template createTemplate(Resource resource, final boolean cacheable) {
        if (resource == null) {
     ...
     */
    @Override
    Template createTemplate(String uri) {
    ...
    Template createTemplateForUri(String uri) {
        return createTemplateForUri([uri] as String[])
    }

    Template createTemplateForUri(String[] uris) {
    ...
    GroovyPageScriptSource findScriptSource(String uri) {
        return findScriptSource([uri] as String[])
    }

    GroovyPageScriptSource findScriptSource(String[] uris) {
    ...
    Template createTemplate(ScriptSource scriptSource) {
     ...
     */
    Template createTemplate(String txt, String pageName) throws IOException {
     ...
     * @throws IOException Thrown if an IO exception occurs creating the Template
     */
    Template createTemplate(String txt, String pageName, boolean cache) throws IOException {
        Assert.hasLength(txt, 'Argument [txt] cannot be null or blank')
        Assert.hasLength(pageName, 'Argument [pageName] cannot be null or blank')
     ...
     */
    @Override
    Template createTemplate(InputStream inputStream) {
    ...
    protected GroovyPageMetaInfo buildPageMetaInfo(Resource resource, String pageName) throws IOException {
        ...
        return observation.observeChecked({ -> doBuildPageMetaInfo(resource, pageName) } as Observation.CheckedCallable<GroovyPageMetaInfo, IOException>)
    }
    ...
    private StringBuilder decorateGroovyPageSource(StringBuilder source) throws IOException {
        for (GroovyPageSourceDecorator groovyPageSourceDecorator in groovyPageSourceDecorators) {
    ...
    void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        if (applicationContext.containsBean(GrailsApplication.APPLICATION_ID)) {
            this.grailsApplication = applicationContext.getBean(GrailsApplication.APPLICATION_ID, GrailsApplication)
    ...
    Map<String, Class<?>> getDomainClassMap() {
