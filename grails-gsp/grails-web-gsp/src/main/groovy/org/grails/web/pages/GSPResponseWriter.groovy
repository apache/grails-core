 ...
 */
@CompileStatic
class GSPResponseWriter extends GrailsRoutablePrintWriter implements EncoderAware, EncodedAppenderFactory {

    protected static final Log LOG = LogFactory.getLog(GSPResponseWriter)
    ...
    static {
        try {
            instantiator = new ObjenesisStd(false).getInstantiatorOf(GSPResponseWriter)
        }
        catch (Exception e) {
    ...
    static GSPResponseWriter getInstance(final ServletResponse response) {
    ...
    private static GSPResponseWriter getInstance(final ServletResponse response, final int max) {
        ...
        streamBuffer.setChunkMinSize(max.intdiv(2))
        ...
        if (!(response instanceof StreamCharBuffer.LazyInitializingWriter)) {
            streamBuffer.connectTo(new StreamCharBuffer.LazyInitializingMultipleWriter() {
                ...
                LazyInitializingWriter[] initializeMultiple(StreamCharBuffer buffer, boolean autoFlush) {
                    StreamCharBuffer.LazyInitializingWriter[] lazyWriters
                    if (CONTENT_LENGTH_COUNTING_ENABLED) {
                        StreamCharBuffer.LazyInitializingWriter countingWriter = {
                        ...
                        } as StreamCharBuffer.LazyInitializingWriter
                        lazyWriters = [countingWriter, lazyResponseWriter] as StreamCharBuffer.LazyInitializingWriter[]
                    }
                    else {
                        lazyWriters = [lazyResponseWriter] as StreamCharBuffer.LazyInitializingWriter[]
                    }
                    return lazyWriters
                }
            }, AUTOFLUSH_ENABLED)
