 ...
 */
@CompileStatic
class CachingGrailsConventionGroovyPageLocator extends GrailsConventionGroovyPageLocator {
    ...
    @Override
    GroovyPageScriptSource findPageInBinding(final String uri, final TemplateVariableBinding binding) {
        if (uri == null) return null

        Callable<GroovyPageScriptSource> updater = new Callable<GroovyPageScriptSource>() {
            GroovyPageScriptSource call() {
                GroovyPageScriptSource scriptSource = superFindPageInBinding(uri, binding)
    ...
    @Override
    GroovyPageScriptSource findPageInBinding(final String pluginName, final String uri, final TemplateVariableBinding binding) {
        if (uri == null || pluginName == null) return null

        Callable<GroovyPageScriptSource> updater = new Callable<GroovyPageScriptSource>() {
            GroovyPageScriptSource call() {
                GroovyPageScriptSource scriptSource = superFindPageInBinding(pluginName, uri, binding)
                if (scriptSource == null) {
                    scriptSource = NULL_SCRIPT
                }
                return scriptSource
    ...
    @Override
    GroovyPageScriptSource findPage(final String uri) {
        if (uri == null) return null

        Callable<GroovyPageScriptSource> updater = new Callable<GroovyPageScriptSource>() {
            GroovyPageScriptSource call() {
                GroovyPageScriptSource scriptSource = superFindPage(uri)
                if (scriptSource == null) {
                    scriptSource = NULL_SCRIPT
                }
                return scriptSource
    ...
    // Bypasses the override so the Callable above invokes the superclass
    // implementation directly rather than recursing back into this class.
    private GroovyPageScriptSource superFindPageInBinding(String uri, TemplateVariableBinding binding) {
        return super.findPageInBinding(uri, binding)
    }

    private GroovyPageScriptSource superFindPageInBinding(String pluginName, String uri, TemplateVariableBinding binding) {
        return super.findPageInBinding(pluginName, uri, binding)
    }

    private GroovyPageScriptSource superFindPage(String uri) {
        return super.findPage(uri)
    }
    ...
    protected GroovyPageScriptSource lookupCache(final GroovyPageLocatorCacheKey cacheKey, Callable<GroovyPageScriptSource> updater) {
        ...
        if (cacheTimeout == 0) {
            ...
            scriptSource = CacheEntry.getValue(uriResolveCache, cacheKey, cacheTimeout, updater, new Callable<CacheEntry>() {
                @Override
                CacheEntry call() throws Exception {
    ...
    private static final class GroovyPageLocatorCacheKey {
        ...
        static final GroovyPageLocatorCacheKey build(final String uri, final String pluginName, final TemplateVariableBinding binding) {
    ...
    void removePrecompiledPage(GroovyPageCompiledScriptSource scriptSource) {
        ...
        for (Map.Entry<GroovyPageLocatorCacheKey, CacheEntry<GroovyPageScriptSource>> entry in
                new HashSet<Map.Entry<GroovyPageLocatorCacheKey, CacheEntry<GroovyPageScriptSource>>>(uriResolveCache.entrySet())) {
