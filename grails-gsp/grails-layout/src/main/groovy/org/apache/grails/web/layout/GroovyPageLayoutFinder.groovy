 ...
 */
@CompileStatic
class GroovyPageLayoutFinder implements ApplicationListener<ContextRefreshedEvent>, Ordered {

    private static final Logger LOG = LoggerFactory.getLogger(GrailsLayoutDecoratorMapper)
    ...
    void setDefaultDecoratorName(String defaultDecoratorName) {
    ...
    Decorator findLayout(HttpServletRequest request, Content page) {
        return findLayout(request, GSPGrailsLayoutPage.content2htmlPage(page))
    }
    ...
    Decorator findLayout(HttpServletRequest request, Page page) {
        ...
        final Object layoutAttribute = request.getAttribute(WebUtils.LAYOUT_ATTRIBUTE)
        if (request.getAttribute(GroovyPagesUriService.RENDERING_VIEW_ATTRIBUTE) != null || layoutAttribute != null) {
            ...
            if (GrailsStringUtils.isBlank(layoutName)) {
                ...
                if (controller != null) {
                    ...
                    if (controllerName != null && actionUri != null) {
                        ...
                        if (LOG.isDebugEnabled()) {
                            LOG.debug('Found controller in request, locating layout for controller [{}] and action [{}}]', controllerName, actionUri)
                        }
    ...
    Decorator getNamedDecorator(HttpServletRequest request, String name, boolean viewMustExist) {
    ...
    void onApplicationEvent(ContextRefreshedEvent event) {
        if (!(viewResolver instanceof GrailsViewResolver)) {
            setViewResolver(event.getApplicationContext().getBean(GrailsViewResolver))
        }
    }
}
