 ...
 */
@CompileStatic
class GrailsLayoutDecoratorMapper extends AbstractDecoratorMapper {

    private GroovyPageLayoutFinder groovyPageLayoutFinder

    @Override
    void init(Config c, Properties properties, DecoratorMapper parentMapper) throws InstantiationException {
        super.init(c, properties, parentMapper)
        ServletContext servletContext = c.getServletContext()
        WebApplicationContext applicationContext = WebApplicationContextUtils.getWebApplicationContext(servletContext)
        if (applicationContext != null) {
            GrailsApplication grailsApplication = applicationContext.getBean(GrailsApplication.APPLICATION_ID, GrailsApplication)
            groovyPageLayoutFinder = grailsApplication.getMainContext().getBean('groovyPageLayoutFinder', GroovyPageLayoutFinder)
        }
    }
