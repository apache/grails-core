...
class CompiledTagCallRewriter extends ClassCodeExpressionTransformer {
    ...
    private static final ClassNode INVOCATION_TYPE = ClassHelper.make(CompiledTagInvocation)
    ...
    private static final String REWRITTEN_MARKER = CompiledTagCallRewriter.name
    ...
    void rewrite() {
        ...
        for (ConstructorNode constructor in classNode.getDeclaredConstructors()) {
        ...
        for (Statement statement in classNode.getObjectInitializerStatements()) {
    ...
    private Expression invocation(String namespace, String tagName, Expression arguments) {
        ...
        for (Expression argument in tuple.getExpressions()) {
    ...
    private Expression[] attributesAndBody(TupleExpression tuple) {
        ...
        switch (args.size()) {
            case 0:
                return [noAttributes, noBody] as Expression[]
            case 1:
                if (args.get(0) instanceof MapExpression) {
                    return [transform(args.get(0)), noBody] as Expression[]
                }
                if (args.get(0) instanceof ClosureExpression) {
                    return [noAttributes, transform(args.get(0))] as Expression[]
                }
                return null
            case 2:
                if (args.get(0) instanceof MapExpression && args.get(1) instanceof ClosureExpression) {
                    return [transform(args.get(0)), transform(args.get(1))] as Expression[]
                }
                return null
            default:
                return null
    ...
    private void validateMarkupTagCall(MethodCallExpression call) {
        if (!page || !MARKUP_TAG_CALL.equals(call.getMethodAsString()) ||
                !(call.getArguments() instanceof TupleExpression)) {
            return
        }
        ...
        if (!(tuple.getExpression(0) instanceof ConstantExpression) ||
                !(tuple.getExpression(1) instanceof ConstantExpression)) {
            return
        }
        ConstantExpression tagName = (ConstantExpression) tuple.getExpression(0)
        ConstantExpression namespace = (ConstantExpression) tuple.getExpression(1)
        if (tagName.getValue() == null || namespace.getValue() == null) {
            return
        }
    ...
    private static String namespaceOf(Expression objectExpression) {
        ...
        if (objectExpression instanceof PropertyExpression) {
            PropertyExpression property = (PropertyExpression) objectExpression
            if (property.getObjectExpression() instanceof VariableExpression) {
                VariableExpression receiver = (VariableExpression) property.getObjectExpression()
    ...
    private boolean hasGetter(String namespace) {
        String capitalised = String.valueOf(Character.toUpperCase(namespace.charAt(0))) + namespace.substring(1)
        if (!classNode.getMethods('get' + capitalised).isEmpty()) {
            return true
        }
        for (MethodNode candidate in classNode.getMethods('is' + capitalised)) {
    ...
    private static Set<String> reservedNames() {
        ...
        names.add('body')
        names.add('render')
        for (MetaMethod method in GroovySystem.getMetaClassRegistry().getMetaClass(Object).getMetaMethods()) {
