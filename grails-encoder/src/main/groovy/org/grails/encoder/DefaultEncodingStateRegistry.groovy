 ...
 */
@CompileStatic
final class DefaultEncodingStateRegistry implements EncodingStateRegistry {

    private static final Logger LOG = LoggerFactory.getLogger(DefaultEncodingStateRegistry)
    ...
    EncodingState getEncodingStateFor(CharSequence string) {
        ...
        for (Map.Entry<Encoder, Map<Long, WeakReference<CharSequence>>> entry : encodedCharSequencesForEncoder.entrySet()) {
            ...
            if (charSequenceReference != null && string.is(charSequenceReference.get())) {
                if (result == null) {
    ...
    void registerEncodedWith(Encoder encoder, CharSequence escaped) {
        ...
        if (previousValue != null && !previousValue.get().is(escaped)) {
    ...
    static boolean isNoneEncoder(Encoder encoderToApply) {
        return encoderToApply == null || encoderToApply.is(NONE_ENCODER) || encoderToApply.getClass() == NoneEncoder
    }
    ...
    static boolean isPreviousEncoderSafeOrEqual(Encoder encoderToApply, Encoder previousEncoder) {
        return previousEncoder.is(encoderToApply) || !encoderToApply.isApplyToSafelyEncoded() && previousEncoder.isSafe() && encoderToApply.isSafe() ||
