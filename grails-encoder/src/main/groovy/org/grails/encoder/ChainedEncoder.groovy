...
@CompileStatic
class ChainedEncoder implements Encoder, StreamingEncoder {
    ...
    private static class ChainedEncoderCacheItem {
        ...
        EncodedAppender getCached(final EncodedAppender appender) {
            if (lastAppenderForCached.is(appender) && lastIgnoreEncodingStateForCached == appender.isIgnoreEncodingState()) {
    ...
    ChainedEncoder(List<StreamingEncoder> encoders, boolean safe) {
        this(encoders.toArray(new StreamingEncoder[encoders.size()]), safe)
    }

    ChainedEncoder(StreamingEncoder[] encoders, boolean safe) {
        this.encoders = Arrays.copyOf(encoders, encoders.length)
        this.combinedCodecIdentifier = createCodecIdentifier(encoders)
        this.safe = safe
    }

    static StreamingEncoder createFor(StreamingEncoder[] encoders) {
        return createFor(Arrays.asList(encoders))
    }

    static StreamingEncoder createFor(List<StreamingEncoder> encoders) {
        return createFor(encoders, null)
    }

    static StreamingEncoder createFor(List<StreamingEncoder> encoders, Boolean safe) {
        if (encoders == null) {
            return null
