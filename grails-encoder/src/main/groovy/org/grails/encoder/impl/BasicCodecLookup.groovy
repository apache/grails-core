...
@CompileStatic
class BasicCodecLookup implements CodecLookup, InitializingBean {
    ...
    Encoder lookupEncoder(String codecName) {
        return lookupCodec(codecName, encoders, Encoder)
    }

    Decoder lookupDecoder(String codecName) {
        return lookupCodec(codecName, decoders, Decoder)
    }
    ...
    protected <T extends CodecIdentifierProvider> T createCodec(String codecName, ConcurrentMap<String, T> map, Class<T> returnType) {
        if (codecName.indexOf((int) (',' as char)) > -1) {
