 ...
 */
@CompileStatic
class HTML4Encoder extends AbstractCharReplacementEncoder {

    private static final Log log = LogFactory.getLog(HTML4Encoder)
    ...
    protected String escapeCharacter(char ch, char previousChar) {
        ...
        return !NULL_MARKER.is(replacement) ? replacement : null
    }
    ...
    private static final class StreamingHTMLEncoderHelper {
        ...
        static {
            try {
                Field instanceField = ReflectionUtils.findField(HtmlUtils, 'characterEntityReferences')
                ReflectionUtils.makeAccessible(instanceField)
                instance = instanceField.get(null)
                mapMethod = ReflectionUtils.findMethod(instance.getClass(), 'convertToReference', char)
                if (mapMethod != null)
         ...
         */
        static final String convertToReference(char c) {
            if (!disabled) {
