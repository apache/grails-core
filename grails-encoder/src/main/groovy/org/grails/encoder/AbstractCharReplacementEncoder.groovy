 ...
 */
@CompileStatic
abstract class AbstractCharReplacementEncoder implements Encoder, StreamingEncoder, EncodesToWriter {
    ...
    protected Object escapeCharSequence(CharSequence str) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (escaped != null) {
                if (sb == null) {
                    sb = new StringBuilder((str.length() * 110).intdiv(100))
                }
                if (i - startPos > 0) {
    ...
    @Override
    EncodesToWriter createChainingEncodesToWriter(List<StreamingEncoder> encoders, boolean applyAdditionalFirst) {
     ...
     */
    void encodeToStream(Encoder thisInstance, CharSequence str, int off, int len, EncodedAppender appender, EncodingState encodingState)
            throws IOException {
        if (str == null || len <= 0) {
            return
        }
