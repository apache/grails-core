...
@AstTransformer
@CompileStatic
class ControllerActionTransformer implements GrailsArtefactClassInjector, AnnotatedClassInjector, CompilationUnitAware {

    public static final AnnotationNode DELEGATING_METHOD_ANNOATION = new AnnotationNode(ClassHelper.make(DelegatingMethod))
    ...
    private static final ClassNode OBJECT_CLASS = new ClassNode(Object)
    private static final ClassNode STRING_CLASS = new ClassNode(String)
    private static final ClassNode SERIALIZABLE_CLASS = new ClassNode(Serializable)
    private static final ClassNode BIND_ALLOWED_CLASS_NODE = new ClassNode(BindAllowed)
    ...
    String[] getArtefactTypes() {
        return [ControllerArtefactHandler.TYPE] as String[]
    }

    void performInjection(SourceUnit source, GeneratorContext context, ClassNode classNode) {
        // don't inject if already an @Artefact annotation is applied
        if (!classNode.getAnnotations(new ClassNode(Artefact)).isEmpty()) return
    ...
    private boolean isExceptionHandlingMethod(MethodNode methodNode) {
        ...
        if (!methodNode.isPrivate() && methodNode.getName().indexOf('\$') == -1) {
            ...
            if (parameters.length == 1) {
                ...
                isExceptionHandler = parameterTypeClassNode.isDerivedFrom(new ClassNode(Exception))
            }
        }
    ...
    private void processMethods(ClassNode classNode, SourceUnit source,
        ...
        if (exceptionHandlerMetaDataField == null || !exceptionHandlerMetaDataField.getDeclaringClass().equals(classNode)) {
            ...
            for (final MethodNode exceptionHandlerMethod in exceptionHandlerMethods) {
                ...
                listOfExceptionHandlerMetaData.addExpression(new ConstructorCallExpression(new ClassNode(DefaultControllerExceptionHandlerMetaData), defaultControllerExceptionHandlerMetaDataCtorArgs))
            }
            classNode.addField(EXCEPTION_HANDLER_META_DATA_FIELD_NAME,
                    Modifier.STATIC | Modifier.PRIVATE | Modifier.FINAL, new ClassNode(List),
                    listOfExceptionHandlerMetaData)
        }

        for (MethodNode newMethod in deferredNewMethods) {
    ...
    protected boolean methodShouldBeConfiguredAsControllerAction(final MethodNode method) {
        int minLineNumber = 0
        if (isInheritedFromTrait(method) && hasAnnotation(method, Action) && hasParameters(method)) {
            removeAnnotation(method, Action)
        ...
        return !method.isStatic() &&
                ...
                method.getAnnotations(new ClassNode(ControllerMethod)).isEmpty() &&
    ...
    protected Collection<MethodNode> getExceptionHandlerMethods(final ClassNode classNode, SourceUnit sourceUnit) {
        ...
        if (!superClass.equals(OBJECT_CLASS)) {
            ...
            for (MethodNode superClassMethod in superClassMethods) {
    ...
    private boolean isCommandObjectAction(Parameter[] params) {
        return params != null && params.length > 0 &&
                params[0].getType() != new ClassNode(Object[]) &&
                params[0].getType() != new ClassNode(Object)
    }
    ...
    private void processClosures(ClassNode classNode, SourceUnit source, GeneratorContext context) {
        ...
        for (PropertyNode property in propertyNodes) {
            initialExpression = property.getInitialExpression()
            if (!property.isStatic() && initialExpression != null &&
                    initialExpression.getClass().equals(ClosureExpression)) {
    ...
    protected void addMethodToInvokeClosure(ClassNode controllerClassNode,
        ...
        if (method == null || !method.getDeclaringClass().equals(controllerClassNode)) {
            ...
            newMethodCode.addStatement(new ExpressionStatement(applyMethodTarget(methodCallExpression, Closure, Object)))

            final MethodNode methodNode = new MethodNode(closureProperty.getName(), Modifier.PUBLIC,
                    new ClassNode(Object), Parameter.EMPTY_ARRAY, ClassNode.EMPTY_ARRAY, newMethodCode)
    ...
    protected void annotateActionMethod(ClassNode controllerClassNode, final Parameter[] parameters, final MethodNode methodNode) {
        ...
        if (isCommandObjectAction(parameters)) {
            ...
            AnnotationNode paramActionAnn = new AnnotationNode(new ClassNode(Action))
    ...
    protected BlockStatement getCodeToHandleAllowedMethods(ClassNode controllerClass, String methodName) {
        ...
        if (allowedMethodsMapExpression != null && isActionRestricted(allowedMethodsMapExpression, methodName)) {
            ...
            final Expression isAllowedMethodCall = new StaticMethodCallExpression(ClassHelper.make(AllowedMethodsHelper), 'isAllowed', isAllowedArgumentList)
    ...
    private MapExpression getAllowedMethodsMapExpression(final ClassNode controllerClass) {
        ...
        allowedMethodsMapExpression.getMapEntryExpressions().isEmpty() ? null : allowedMethodsMapExpression
    }
    ...
    private boolean isActionRestricted(final MapExpression allowedMethodsMapExpression, final String methodName) {
        for (MapEntryExpression allowedMethodsMapEntryExpression in allowedMethodsMapExpression.getMapEntryExpressions()) {
    ...
    protected void wrapMethodBodyWithExceptionHandling(final ClassNode controllerClassNode, final MethodNode methodNode) {
        ...
        final ClassNode reflectMethodClassNode = new ClassNode(Method)
        final String exceptionHandlerMethodVariableName = '\$method'
        final Expression exceptionHandlerMethodExpression = new VariableExpression(exceptionHandlerMethodVariableName, new ClassNode(Method))
        ...
        final CatchStatement catchStatement = new CatchStatement(new Parameter(new ClassNode(Exception), caughtExceptionArgumentName), catchBlockCode)
        ...
        final VariableExpression attributeValueExpression = new VariableExpression('\$allowed_methods_attribute_value', ClassHelper.make(Object))
        ...
        tryCatchToRemoveAttribute.addCatch(new CatchStatement(new Parameter(ClassHelper.make(Exception), '\$exceptionRemovingAttribute'), new EmptyStatement()))
    ...
    protected BlockStatement initializeActionParameters(ClassNode classNode, ASTNode actionNode,
        ...
        mapBindingResultConstructorArgs.addExpression(new ConstructorCallExpression(
                new ClassNode(HashMap), EMPTY_TUPLE))
        ...
        final Expression mapBindingResultConstructorCallExpression = new ConstructorCallExpression(
                new ClassNode(MapBindingResult), mapBindingResultConstructorArgs)
        ...
        if (actionParameters != null) {
            for (Parameter param in actionParameters) {
    ...
    protected void initializeMethodParameter(final ClassNode classNode, final BlockStatement wrapper,
        ...
        List<AnnotationNode> requestParameters = param.getAnnotations(
                new ClassNode(RequestParameter))
        ...
        if (actionNode instanceof MethodNode && paramName.startsWith('arg')) {
            List<AnnotationNode> traitBridges = ((MethodNode) actionNode).getAnnotations(new ClassNode(Traits.TraitBridge))
                ...
                if (traitClass instanceof ClassExpression) {
                    ...
                    if (methods.size() == 1) {
                        ...
                        if (parameters.length >= argNum + 1) {
                            ...
                            requestParameters = helperParam.getAnnotations(new ClassNode(RequestParameter))
    ...
    protected void initializeAndValidateCommandObjectParameter(final BlockStatement wrapper,
        ...
        if (commandObjectNode.isInterface() || Modifier.isAbstract(commandObjectNode.getModifiers())) {
            ...
            boolean argumentIsValidateable = GrailsASTUtils.hasAnyAnnotations(
                    ...
                    commandObjectNode.implementsInterface(ClassHelper.make(Validateable))
            ...
            if (!argumentIsValidateable && commandObjectNode.isPrimaryClassNode()) {
                ...
                if (commandObjectModule != null && this.compilationUnit != null) {
                    if (commandObjectModule == controllerNode.getModule() ||
                        ...
                        TraitInjectionUtils.injectTrait(compilationUnit, source, commandObjectNode, Validateable)
    ...
    private ListExpression copyStringLiteralList(final ListExpression sourceList) {
        final ListExpression listExpression = new ListExpression()
        for (Expression element in sourceList.getExpressions()) {
    ...
    protected void initializeStringParameter(final ClassNode classNode, final BlockStatement wrapper, final Parameter param,
        ...
        final BooleanExpression containsKeyExpression = boolX(applyDefaultMethodTarget(callX(getParamsExpression, 'containsKey', args(constX(requestParameterName))), Map))
    ...
    protected void initializePrimitiveOrTypeWrapperParameter(final ClassNode classNode, final BlockStatement wrapper,
        ...
        if ('char'.equals(conversionMethodName)) {
            ...
            defaultValueClass = Integer
        }
        applyMethodTarget(retrieveConvertedValueExpression, TypeConvertingMap, null, defaultValueClass)
        ...
        final BooleanExpression containsKeyExpression = new BooleanExpression(
                applyDefaultMethodTarget(new MethodCallExpression(getParamsExpression, 'containsKey', paramsContainsKeyMethodArguments), Map))
        ...
        final VariableExpression convertedValueExpression = new VariableExpression(
                '___converted_' + methodParamName, new ClassNode(Object))
    ...
    protected Expression getRejectValueExpression(final ClassNode classNode, final String methodParamName) {
        ...
        Expression rejectValueMethodCallExpression = applyDefaultMethodTarget(new MethodCallExpression(
                getErrorsExpression, 'rejectValue', rejectValueArgs), Errors)
