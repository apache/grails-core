 ...
 */
@CompileStatic
class ControllerRuntimeHints implements RuntimeHintsRegistrar {
    ...
    void registerHints(RuntimeHints hints, @Nullable ClassLoader classLoader) {
        for (Class<?> type in DISPATCHED_TYPES) {
            hints.reflection().registerType(type, DISPATCHED_MEMBERS)
        }
        for (String type in OPTIONAL_DISPATCHED_TYPES) {
