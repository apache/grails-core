...
interface ConstrainedProperty extends Constrained {
    ...
    // Built with a Groovy map literal rather than an anonymous-HashMap-with-instance-initializer.
    // On Groovy 5 (GROOVY-12063) the bareword references to the sibling DEFAULT_* constants inside
    // such an initializer compile to dynamic getProperty calls on `this` (the HashMap), which the
    // MOP resolves as map-key lookups on the still-empty map, so every entry was captured as null.
    // A map literal resolves the constants against the enclosing interface scope and is correct on
