 ...
 */
@CompileStatic
class DefaultConstraintEvaluator implements ConstraintsEvaluator {

    private static final Logger LOG = LoggerFactory.getLogger(DefaultConstraintEvaluator)
    ...
    DefaultConstraintEvaluator() {
        this(new DefaultConstraintRegistry(new StaticMessageSource()), new KeyValueMappingContext('default'), Collections.emptyMap(), true)
    }
    ...
    DefaultConstraintEvaluator(MessageSource messageSource, MappingContext mappingContext) {
        this(new DefaultConstraintRegistry(messageSource), mappingContext, Collections.emptyMap(), true)
    }

    DefaultConstraintEvaluator(MessageSource messageSource, MappingContext mappingContext, Map<String, Object> defaultConstraints) {
        this(new DefaultConstraintRegistry(messageSource), mappingContext, defaultConstraints, true)
    }

    DefaultConstraintEvaluator(MessageSource messageSource) {
        this(new DefaultConstraintRegistry(messageSource), new KeyValueMappingContext('default'), Collections.emptyMap(), true)
    }

    DefaultConstraintEvaluator(ConstraintRegistry constraintRegistry, MappingContext mappingContext, Map<String, Object> defaultConstraints) {
        this(constraintRegistry, mappingContext, defaultConstraints, true)
    }

    DefaultConstraintEvaluator(ConstraintRegistry constraintRegistry, MappingContext mappingContext, Map<String, Object> defaultConstraints, boolean cacheAutoTimestampAnnotations) {
        this(constraintRegistry, mappingContext, defaultConstraints, cacheAutoTimestampAnnotations, true)
    }

    DefaultConstraintEvaluator(ConstraintRegistry constraintRegistry, MappingContext mappingContext, Map<String, Object> defaultConstraints, boolean cacheAutoTimestampAnnotations, boolean defaultNullable) {
    ...
    @Override
    Map<String, ConstrainedProperty> evaluate(@SuppressWarnings('rawtypes') Class theClass, boolean defaultNullable) {
        return evaluate(theClass, defaultNullable, false)
    }

    @Override
    Map<String, ConstrainedProperty> evaluate(Class<?> theClass, boolean defaultNullable, boolean useOnlyAdHocConstraints, Closure... adHocConstraintsClosures) {
        List<Closure> constraints = useOnlyAdHocConstraints ? new ArrayList<>() : ClassPropertyFetcher.getStaticPropertyValuesFromInheritanceHierarchy(theClass, PROPERTY_NAME, Closure)
        ...
        if (entity != null) {
            ...
            if (properties != null) {
                ...
                for (PersistentProperty p in properties) {
        ...
        if (properties == null || properties.size() == 0) {
            ...
            for (MetaProperty metaProperty in metaProperties) {
                ...
                if (!constrainedProperties.containsKey(propertyName) && NameUtils.isNotConfigurational(propertyName)) {
                    ...
                    if (metaProperty instanceof MetaBeanProperty) {
                        ...
                        if (Boolean == propertyType && getter != null && getter.getName().startsWith('is')) {
                            continue
                        }
                    }
                    if (!defaultNullable) {
    ...
    protected void applyDefaultNullableConstraint(ConstrainedProperty cp, boolean defaultNullable) {
        boolean isCollection = Collection.isAssignableFrom(cp.getPropertyType()) || Map.isAssignableFrom(cp.getPropertyType())
    ...
    protected boolean canApplyNullableConstraint(String propertyName, PersistentProperty property, Constrained constrained) {
        ...
        if (!GroovyObject.isAssignableFrom(domainClass.getJavaClass())) return false
