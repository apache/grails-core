...
@CompileStatic
class UrlMappingsHolderFactoryBean implements FactoryBean<UrlMappings>, InitializingBean, ApplicationContextAware, GrailsApplicationAware {
    ...
    UrlMappings getObject() throws Exception {
        return urlMappingsHolder
    }

    Class<UrlMappings> getObjectType() {
        return UrlMappings
    }
    ...
    void afterPropertiesSet() throws Exception {
        ...
        if (mappings.length == 0) {
            ...
            for (int i = 0; i < mappings.length; i++) {
                ...
                if (Script.isAssignableFrom(mappingClass.getClazz())) {
                ...
                if (StringUtils.hasLength(mapping.getPluginName())) {
                    for (UrlMapping grailsClassMapping in grailsClassMappings) {
        ...
        Integer cacheSize = config.getProperty(URL_MAPPING_CACHE_MAX_SIZE, Integer, null)
        if (cacheSize != null) {
            defaultUrlMappingsHolder.setMaxWeightedCacheCapacity(cacheSize)
        }
        Integer urlCreatorCacheSize = config.getProperty(URL_CREATOR_CACHE_MAX_SIZE, Integer, null)
        ...
        UrlConverter urlConverter = applicationContext.containsBean(UrlConverter.BEAN_NAME) ? applicationContext.getBean(UrlConverter.BEAN_NAME, UrlConverter) : null
        ...
        ((ConfigurableApplicationContext) applicationContext).addApplicationListener(new ApplicationListener<ArtefactAdditionEvent>() {
            @Override
            void onApplicationEvent(ArtefactAdditionEvent event) {
