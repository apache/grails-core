...
@CompileStatic
class DefaultUrlMappingsHolder implements UrlMappings {

    private static final transient Logger LOG = LoggerFactory.getLogger(DefaultUrlMappingsHolder)
    ...
    private enum CustomListWeigher implements Weigher<UriToUrlMappingKey, List<UrlMappingInfo>> {
        INSTANCE
        @Override
        int weigh(UriToUrlMappingKey key, List<UrlMappingInfo> value) {
            return value.size() + 1
        }
    }
    ...
    void initialize() {
        ...
        for (UrlMapping mapping in mappings) {
    ...
    UrlCreator getReverseMapping(final String controller, final String action, Map params) {
     ...
     */
    UrlCreator getReverseMapping(final String controller, final String action, final String namespace, final String pluginName, Map params) {
    ...
    private UrlCreator resolveUrlCreator(final String controller,
        ...
        if (mapping == null || (mapping instanceof ResponseCodeUrlMapping)) {
            UrlMappingKey lookupKey = new UrlMappingKey(controller, action, namespace, pluginName, httpMethod, version, Collections.emptySet())
        ...
        if (mapping == null || (mapping instanceof ResponseCodeUrlMapping)) {
            ...
            Set<String> paramKeys = new HashSet<String>((Set<String>) params.keySet())
    ...
    protected UrlMapping lookupMapping(String controller, String action, String namespace, String pluginName, String httpMethod, String version, Map params) {
        ...
        Set<String> lookupParams = new HashSet<String>((Set<String>) params.keySet())
        if (secondAttempt) {
    ...
    private static UrlMappingInfo matchMapping(UrlMapping mapping, String uri, int uriSlashCount) {
        if (mapping instanceof RegexUrlMapping) {
            return ((RegexUrlMapping) mapping).match(uri, uriSlashCount)
        }
    ...
    private boolean isExcluded(String uri) {
        if (excludePatterns != null) {
            for (Object excludePattern in excludePatterns) {
    ...
    Set<HttpMethod> allowedMethods(String uri) {
        ...
        for (UrlMappingInfo urlMappingInfo in urlMappingInfos) {
    ...
    class UrlMappingKey implements Comparable {
        ...
        UrlMappingKey(String controller, String action, String namespace, String pluginName, String httpMethod, String version, Set<String> paramNames) {
    ...
    class UrlMappingsList {
        ...
        void put(UrlMappingsListKey key, UrlMappingKey mapping) {
        ...
        Collection<UrlMappingKey> get(UrlMappingsListKey key) {
