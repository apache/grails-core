...
class WatchServiceDirectoryWatcher extends AbstractDirectoryWatcher {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(WatchServiceDirectoryWatcher)
        ...
        while (active) {
            try {
                ...
                if (watchKey != null) {
                    ...
                    for (WatchEvent<?> watchEvent in watchEvents) {
    ...
    @Override
    void addWatchDirectory(File dir, final List<String> fileExtensions) {
        ...
        try {
            ...
            Files.walkFileTree(dir, new SimpleFileVisitor<Path>() {
                ...
                FileVisitResult preVisitDirectory(Path subDir, BasicFileAttributes attrs)
                        throws IOException {
                    if (!isValidDirectoryToMonitor(subDir.toFile())) {
                        return FileVisitResult.SKIP_SUBTREE
                    }
                    WatchKey watchKey = subDir.register(watchService, StandardWatchEventKinds.ENTRY_CREATE, StandardWatchEventKinds.ENTRY_DELETE, StandardWatchEventKinds.ENTRY_MODIFY)
                    ...
                    if (originalFileExtensions == null) {
                        ...
                        watchKeyToExtensionsMap.put(watchKey, Collections.unmodifiableList(new ArrayList<String>(newFileExtensions)))
                    }
