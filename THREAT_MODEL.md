...
**Secondary intended use**: project scaffolding and code generation via the CLI tools and the hosted `grails-forge` HTTP APIs. The browser UI and redirect target are at <https://start.grails.org>. *(documented: [README.md](./README.md))*
...
| Hosted `grails-forge` HTTP APIs | [`ZipCreateController`](./grails-forge/grails-forge-web/grails-app/controllers/org/grails/forge/web/ZipCreateController.groovy) and sibling controllers | Yes - network | **Out** - separate Grails 8 web application deployed independently from user applications built with Grails; modeled separately by the Forge service operators. *(inferred)* |
...
- **Hosted `grails-forge` HTTP service**, including the `ZipCreateController` endpoints. The Grails 8 web application is deployed independently, has its own operator, and is modeled separately. The Grails framework neither runs this service inside user applications nor inherits its risk. *(inferred)*
